...
namespace internal {
    ...
    inline std::byte*
    do_scan(std::byte* begin, std::byte* end, signature sig, scan_mode mode) {
...
class scanner {
  public:
    ...
    [[nodiscard]]
    memory_range_element_t<Range>* scan_signature(
        signature sig,
        scan_mode mode = scan_mode::automatic
    ) const noexcept {
        ...
        for (auto& i : range_) {
            ...
            if (auto result = internal::do_scan(
                    std::to_address(i.begin()),
                    std::to_address(i.end()),
                    sig,
                    mode
                );
                result)
