...
struct sig_element {
    ...
    constexpr explicit sig_element(
        std::byte byte,
    ...
    [[nodiscard]]
    constexpr std::byte byte() const noexcept {
        return byte_;
    }

    [[nodiscard]]
    constexpr std::byte mask() const noexcept {
        return mask_;
    }

    friend constexpr bool
    operator==(const sig_element& lhs, std::byte rhs) noexcept {
...
template<size_t N>
class static_sig_storage:
    public internal::sig_base<std::array<sig_element, N>> {
  public:
...
class signature: public internal::sig_base<std::span<const sig_element>> {
  ...
  public:
    ...
    constexpr signature(
        const sig_storage& sig
    ) : // NOLINT(google-explicit-constructor)
        base_t(container_t {sig.container()}) {}

    template<size_t N>
    constexpr signature(const static_sig_storage<N>& sig) noexcept
        : // NOLINT(google-explicit-constructor)
        base_t(container_t {sig.container()}) {}

    constexpr explicit signature(std::span<const sig_element> span) noexcept :
        base_t(span) {}
    ...
    [[nodiscard]]
    constexpr auto subsig(size_t offset, size_t count = std::dynamic_extent) {
...
constexpr sig_storage parse_signature(std::string_view str) {
    ...
    while (iter < str.size()) {
        ...
        result.container().push_back(
            internal::parse_byte(str.substr(iter, size))
        );
...
consteval auto parse_static_signature() {
    constexpr size_t size =
        parse_signature(Str.stringview()).container().size();
