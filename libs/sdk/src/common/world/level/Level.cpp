...
void SDK::Level::playSoundEvent(
    std::string const& text,
    Vec3 const& pos,
    float vol,
    float pitch
) {
    static int index = mvGetOffset<
        0xB4,
        0xB3,
        0xB2,
        0xB2,
...
std::unordered_map<UUID, SDK::PlayerListEntry>* SDK::Level::getPlayerList() {
    if (internalVers >= SDK::V1_21_40) {
        return *reinterpret_cast<
            std::unordered_map<UUID, SDK::PlayerListEntry>**>(
            reinterpret_cast<uintptr_t>(this) + 0x960
        );
    }

    static int index =
        SDK::mvGetOffset<0x112, 0x111, 0x120, 0x120, 0x128, 0x137>();
    return memory::callVirtual<std::unordered_map<UUID, SDK::PlayerListEntry>*>(
        this,
        index
    );
}
...
SDK::HitResult* SDK::Level::getLiquidHitResult() {
    ...
    return reinterpret_cast<SDK::HitResult*>(
        memory::callVirtual<uintptr_t>(this, index)
    ) /*sizeof hitResult (0x60) / 8*/;
}
