...
void SDK::Player::displayClientMessage(std::string const& message) {
    if (internalVers >= V1_21_20) {
        ...
        memory::callVirtual<void>(
            this,
            mvGetOffset<0xC8, 0xCD, 0xCE, 0xD2, 0xD2, 0x0, 0x0, 0x0, 0x0, 0x0>(
            ),
            message,
            opt
        );
    ...
    memory::callVirtual<void>(
        this,
        mvGetOffset<0xEF, 0x15E, 0x184, 0x18A>(),
        message
    );
}
...
SDK::MoveInputComponent* SDK::Player::getMoveInputComponent() {
    ...
    using try_get_t =
        SDK::MoveInputComponent*(__fastcall*)(uintptr_t a1, uint32_t* a2);
    static auto func = reinterpret_cast<try_get_t>(
        Signatures::Components::moveInputComponent.result
    );
    return func(entityContext.getBasicRegistry(), &entityContext.getId());
}

std::string SDK::Player::getXUID() {
    return memory::callVirtual<std::string>(
        this,
        SDK::mvGetOffset<
            0xEC,
            0xF1,
