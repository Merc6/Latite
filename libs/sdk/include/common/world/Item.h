  ...
  public:
    ...
    bool isGlint(class ItemStackBase* item) {
        return memory::callVirtual<bool>(
            this,
            mvGetOffset<0x27, 0x26, 0x26, 0x26, 0x32, 0x32, 0x2B, 0x32>(),
            item
        );
    }
    ...
    int getMaxDamage() {
        return memory::callVirtual<int>(
            this,
            mvGetOffset<0x23, 0x22, 0x22, 0x22, 0x2F, 0x2F, 0x28, 0x2F>()
        );
    }
    ...
    short getDamageValue(class CompoundTag* tag) {
        ...
        if (internalVers >= V1_20_50) {
            return reinterpret_cast<short (*)(
                Item*,
                CompoundTag*
            )>(Signatures::ItemStackBase_getDamageValue.result)(this, tag);
        }

        return memory::callVirtual<short>(
            this,
            mvGetOffset<0x6B, 0x65, 0x6B>(),
            tag
        );
    }
