...
bool memory::signature_store::resolve() {
    ...
    auto* const dosHeader =
        reinterpret_cast<PIMAGE_DOS_HEADER>(GetModuleHandleA(0));
    ...
    mnem::scanner scanner {
        mnem::memory_span {(std::byte*)scanBytes, (size_t)sizeOfCode}
    };

    scan_result = reinterpret_cast<uintptr_t>(
        scanner.scan_signature(this->signature.value())
    ); //memory::findSignature(this->signature, mod);
    if (!scan_result)
        return false;
