...
int getCountStr(SDK::Inventory* inv, std::string incl) {
    ...
    for (int i = 0; i < 36; i++) {
        ...
        if (auto it = stack->item) {
            ...
            if (tName.find(incl) != std::string::npos)
...
ItemCounter::ItemCounter() :
    HUDModule(
        "ItemCounter",
        LocalizeString::get("client.hudmodule.itemCounter.name"),
        LocalizeString::get("client.hudmodule.itemCounter.desc"),
        HUD
    ) {
    ...
    addSetting(
        "alwaysShow",
        LocalizeString::get("client.hudmodule.itemCounter.alwaysShow.name"),
        L"",
        alwaysShow
    );
    addSetting(
        "potions",
        LocalizeString::get("client.hudmodule.itemCounter.potions.name"),
        L"",
        potions
    );
    addSetting(
        "arrows",
        LocalizeString::get("client.hudmodule.itemCounter.arrows.name"),
        L"",
        arrow
    );
    addSetting(
        "crystals",
        LocalizeString::get("client.hudmodule.itemCounter.crystals.name"),
        L"",
        crystals
    );
    addSetting(
        "totems",
        LocalizeString::get("client.hudmodule.itemCounter.totems.name"),
        L"",
        totems
    );
    addSetting(
        "bottles",
        LocalizeString::get("client.hudmodule.itemCounter.xpBottles.name"),
        L"",
        xpBottles
    );
...
void ItemCounter::render(DrawUtil& ct, bool isDefault, bool inEditor) {
    ...
    for (auto& counters : counterList) {
        if (std::get<BoolValue>(counters.setting) == false)
            continue;
        int count = counters.isHash ? getCountHashed(inv, counters.hash)
                                    : getCountStr(inv, counters.inString);

        if (count || std::get<BoolValue>(alwaysShow)) {
            SDK::TexturePtr text {};
            ctx->getTexture(
                &text,
                SDK::ResourceLocation(counters.texture.c_str(), 0),
                false
            );
            ...
            d2d::Rect textRect =
                {rc.right + 2.f, rc.top, rc.right + 2.f + txtSize.x, rc.bottom};
