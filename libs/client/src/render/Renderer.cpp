...
bool Renderer::init(IDXGISwapChain* chain) {
    ...
    bool isDX12 = true;
    if (!dx12Removed && SUCCEEDED(chain->GetDevice(IID_PPV_ARGS(&gameDevice12)))
        && Latite::get().shouldForceDX11()) {
    ...
    if (SUCCEEDED(chain->GetDevice(IID_PPV_ARGS(&gameDevice12))) && !dx12Removed
        && Latite::get().shouldForceDX11()) {
        static_cast<ID3D12Device5*>(gameDevice12.Get())->RemoveDevice();
        bufferCount = 1;
        Logger::Info("Force DX11 active");
    ...
    if (!gameDevice11.Get() || !gameDevice12.Get()) {
        if (Latite::get().shouldForceDX11()
            || FAILED(chain->GetDevice(IID_PPV_ARGS(&gameDevice12)))) {
...
#pragma warning(pop)
        ...
        ID2D1Bitmap1* targ;
        ThrowIfFailed(
            d2dCtx->CreateBitmapFromDxgiSurface(surf.Get(), &prop, &targ)
        );
    ...
    ThrowIfFailed(d2dCtx->CreateBitmap(
        bitmapSize,
        nullptr,
        0,
...
void Renderer::render() {
    ...
    deltaTime = std::clamp(
        static_cast<float>(diff.count()) / 17.f,
        0.02f,
        20.f
    ); // based on 60-ish FPS
...
void Renderer::createDeviceDependentResources() {
    ...
    ThrowIfFailed(d2dCtx->CreateEffect(
        CLSID_D2D1Shadow,
        this->shadowEffect.GetAddressOf()
    ));
    ThrowIfFailed(d2dCtx->CreateEffect(
        CLSID_D2D12DAffineTransform,
        this->affineTransformEffect.GetAddressOf()
    ));
    ThrowIfFailed(d2dCtx->CreateEffect(
        CLSID_D2D1GaussianBlur,
        this->blurEffect.GetAddressOf()
    ));
}
...
IDWriteTextLayout* Renderer::getLayout(
    ...
    return layout.Get(
    ); // Im pretty sure it implicitly adds a ref when I add it to cachedLayouts
}
