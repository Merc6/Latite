...
int OptionHooks::Options_getPerspective(void* options) {
    auto o =
        getPerspectiveHook->oFunc<decltype(&Options_getPerspective)>()(options);
...
float OptionHooks::Options_getSensitivity(
    void* options,
    unsigned int a2,
    int a3,
    int a4,
    ...
    auto o = getSensitivityHook->oFunc<decltype(&Options_getSensitivity)>()(
        options,
        a2,
        a3,
        a4,
...
OptionHooks::OptionHooks() {
    getGammaHook = addHook(
        Signatures::Options_getGamma.result,
        Options_getGamma,
        "Options::getGamma"
    );
    getPerspectiveHook = addHook(
        Signatures::Options_getPerspective.result,
        Options_getPerspective,
        "Options::getPerspective"
    );
    getHideHandHook = addHook(
        Signatures::Options_getHideHand.result,
        Options_getHideHand,
        "Options::getHideHand"
    );
