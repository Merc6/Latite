...
HUDEditor::HUDEditor() : dragMod(nullptr) {
    ...
    Eventing::get().listen<RenderOverlayEvent>(
        this,
        (EventListenerFunc)&HUDEditor::onRender,
        2,
        true
    );
    Eventing::get().listen<RenderLayerEvent>(
        this,
        (EventListenerFunc)&HUDEditor::onRenderLayer,
        1,
        true
    );
    Eventing::get()
        .listen<ClickEvent>(this, (EventListenerFunc)&HUDEditor::onClick, 4);
    Eventing::get().listen<KeyUpdateEvent>(
        this,
        (EventListenerFunc)&HUDEditor::onKey
    );
}
...
void HUDEditor::onRender(Event& ev) {
    ...
    if (isActive()) {
        Latite::getModuleManager().forEach([&](std::shared_ptr<IModule> mod) {
            if (mod->isHud() && mod->isEnabled()) {
                ...
                if (Latite::get().getMenuBlur()
                    && (mcRenderer || rMod->forceMinecraftRenderer()))
        ...
        if (Latite::get().getMenuBlur())
            dc.drawGaussianBlur(toBlur * anim);
        ...
        // Center Button
        {
            ...
            d2d::Rect btnRect = {
                btnPos,
                Vec2(btnPos.x + buttonWidth, btnPos.y + buttonHeight)
            };
    ...
    if (!mcRenderer) {
        ...
        keepModulesInBounds(Vec2(
            Latite::getRenderer().getScreenSize().width,
...
void HUDEditor::onClick(Event& evGeneric) {
    ...
    Latite::getModuleManager().forEach([&](std::shared_ptr<IModule> mod) {
        ...
        if (!shouldSelect(
                hudMod->getRect(),
                SDK::ClientInstance::get()->cursorPos
            ))
            return;
        ev.setCancelled(true);

        if (ev.getMouseButton() == 4) {
            if (!hudMod->isResizable())
                return;
            hudMod->setScale(
                std::clamp(
                    hudMod->getScale()
                        - static_cast<float>(ev.getWheelDelta()) / 1000.f,
                    HUDModule::min_scale,
                    HUDModule::max_scale
                )
            );
            ...
            Latite::getScreenManager().get<ClickGUI>().jumpToModule(
                hudMod->name()
            );
            close();
    ...
    if (ev.getScreenView()->visualTree->rootControl->name == "debug_screen") {
        if (isActive()
            ...
            Latite::getModuleManager().forEach([&](std::shared_ptr<IModule> mod
                ...
                if (mod->isHud() && mod->isEnabled()
                    && reinterpret_cast<HUDModule*>(mod.get())->isActive()
                    ...
                    if (rMod->getCategory() == Module::SCRIPT) {
                        if (!rMod->isTextual()) {
                            auto jsRMod =
                                reinterpret_cast<JsHUDModule*>(mod.get());

                            auto oMat =
                                jsRMod->script->getObject<D2DScriptingObject>()
                                    ->getMatrix();
                            jsRMod->script->getObject<D2DScriptingObject>()
                                ->setMatrix(
                            ...
                            auto jsRMod =
                                reinterpret_cast<JsTextModule*>(mod.get());
...
void HUDEditor::renderModules(
    SDK::MinecraftUIRenderContext* ctx,
    bool forceMinecraftOnly
) {
    if (!isActive()) {
    ...
    if (isActive()
        ...
        Latite::getModuleManager().forEach([&](std::shared_ptr<IModule> mod) {
            ...
            if (mod->isHud() && mod->isEnabled()
                ...
                hudModule->storePos(
                    ctx ? SDK::ClientInstance::get()->getGuiData()->screenSize
                        : Vec2(
                              Latite::getRenderer().getScreenSize().width,
                              Latite::getRenderer().getScreenSize().height
                          )
...
void HUDEditor::renderModule(
    ...
    if (!ctx) {
        ...
        dc.ctx->SetTransform(
            D2D1::Matrix3x2F::Scale(mod->getScale(), mod->getScale())
            * D2D1::Matrix3x2F::Translation(
                mod->getRect().left,
                mod->getRect().top
            )
        ...
        dc.setImmediate(false);
        dc.flush();
        dc.scn->matrix->matrixStack.push(
            D2D1::Matrix4x4F::Scale(mod->getScale(), mod->getScale(), 0.f)
            * D2D1::Matrix4x4F::Translation(
                mod->getRect().left * dc.guiScale,
                mod->getRect().top * dc.guiScale,
                0.f
            )
        );
        mod->render(dc, false, isActive());
...
void HUDEditor::doSnapping(Vec2 const&) {
    ...
    std::vector<float> snapLinesX =
        {0.f, ss.x / 4.f, ss.x / 2.f, ss.x / 2 + (ss.x / 4), ss.x};
    ...
    for (auto& rec : controls) {
        if (rec.left > 0.f && rec.bottom < ss.y)
            snapLinesControlsX.emplace_back(rec.left, rec.top);
        if (rec.right > 0.f && rec.bottom < ss.y)
            snapLinesControlsX.emplace_back(rec.right, rec.top);
        if (rec.top > 0.f && rec.bottom < ss.y)
            snapLinesControlsY.push_back(rec.top);
        if (rec.bottom > 0.f && rec.bottom < ss.y)
            snapLinesControlsY.push_back(rec.bottom);
    }
    ...
    if (isActive() && dragMod && Latite::get().getDoSnapLines()) {
        ...
        for (int i = 0; i < snapLinesY.size(); i++) {
            ...
            if (std::abs(pos.y - pred.right) < snapRange) {
                ...
                dc.brush->SetColor(col.get());
                dc.ctx
                    ->DrawLine({0.f, snap}, {ss.x, snap}, dc.brush, thickness);
            ...
            if (mod->isHud() && mod->isEnabled()) {
                ...
                if (snapX.doSnapping) {
                    if (snapX.type != SnapValue::Module) {
                        ...
                        if (type == Snapping::MCUI
                            && snapLinesControlsX.size() > 0) {
...
void HUDEditor::keepModulesInBounds(Vec2 const& ss) {
    Latite::getModuleManager().forEach([&](std::shared_ptr<IModule> mod) {
        if (mod->isEnabled() && mod->isHud()) {
            ...
            auto round2 = [](float& f) { f = std::round(f); };
