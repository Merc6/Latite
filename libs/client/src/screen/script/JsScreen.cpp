...
JsScreen::JsScreen(JsValueRef object, std::string name) :
    ...
    Eventing::get().listen<RenderLayerEvent>(
        this,
        (EventListenerFunc)&JsScreen::onRender,
        1,
        true
    );
    Eventing::get()
        .listen<KeyUpdateEvent>(this, (EventListenerFunc)&JsScreen::onKey, 1);
    Eventing::get()
        .listen<ClickEvent>(this, (EventListenerFunc)&JsScreen::onClick, 1);
...
void JsScreen::onClick(::Event& evG) {
    ...
    Event sEv {
        L"mouse",
        {Chakra::MakeInt(ev.getMouseButton()),
         Chakra::MakeInt(ev.getWheelDelta()),
         ev.isDown() ? Chakra::GetTrue() : Chakra::GetFalse()}
    };
    auto ret = dispatchEvent(sEv.name, sEv);
    if (ret != JS_INVALID_REFERENCE) {
...
void JsScreen::onEnable(bool ignoreAnims) {
    Latite::get().queueForClientThread([this, ignoreAnims]() {
        ...
        Event ev {
            L"enable",
            {(ignoreAnims ? Chakra::GetTrue() : Chakra::GetFalse())}
        };
        auto ret = dispatchEvent(ev.name, ev);
        if (ret != JS_INVALID_REFERENCE) {
            Chakra::Release(ret);
