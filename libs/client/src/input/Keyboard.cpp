...
Keyboard::Keyboard(int* gameKeyMap) : keyMap(gameKeyMap) {
    Eventing::get()
        .listen<KeyUpdateEvent>(this, (EventListenerFunc)&Keyboard::onKey, 4);
}

void Keyboard::findTextInput() {
    bool backspaceHeld = keyMapAdjusted['\b'];
    static std::chrono::high_resolution_clock::time_point backspaceStart =
    ...
    for (int i = 0; i < 256; i++) {
        ...
        if (isDown && !oldIsDown && i != VK_ESCAPE) {
            ...
            if (keyMapAdjusted[VK_CONTROL]) {
                ...
                int res =
                    ToUnicode(i, dwScanCode, winKeyMap, (LPWSTR)&dwChars, 4, 0);
                if (res > 0) {
...
int Keyboard::getMappedKey(std::string const& name) {
    return SDK::ClientInstance::get()
        ->inputHandler->mappingFactory->defaultKeyboardLayout->findValue(name);
}
