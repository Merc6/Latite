...
void GameScriptingObject::initialize(JsContextRef ctx, JsValueRef parentObj) {
    ...
    Chakra::SetPropertyString(
        object,
        XW("version"),
        util::StrToWStr(Latite::get().gameVersion)
    );
    ...
    Chakra::DefineFunc(
        object,
        getConnectedFeaturedServerCallback,
        XW("getFeaturedServer")
    );
...
void GameScriptingObject::createWorldObject() {
    ...
    Chakra::SetProperty(
        Chakra::GetGlobalObject(),
        XW("dimension"),
        dimensionObj
    );
}

JsValueRef GameScriptingObject::worldExists(
    JsValueRef callee,
    bool isConstructor,
    ...
    return SDK::ClientInstance::get()->minecraft->getLevel()
        ? Chakra::GetTrue()
        : Chakra::GetFalse();
}

JsValueRef GameScriptingObject::worldGetName(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    return Chakra::MakeString(
        util::StrToWStr(
            SDK::ClientInstance::get()->minecraft->getLevel()->getLevelName()
        )
    );
}

JsValueRef GameScriptingObject::worldGetPlayers(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    JsValueRef array;
    auto arr =
        SDK::ClientInstance::get()->minecraft->getLevel()->getPlayerList();
    auto size = arr->size();
    ...
    for (auto& pair : *arr) {
        JS::JsSetIndexedProperty(
            array,
            Chakra::MakeInt(i),
            Chakra::MakeString(util::StrToWStr(pair.second.name))
        );
        ++i;
    }
    return array;
...
JsValueRef GameScriptingObject::worldGetEntList(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Latite::getPluginManager()
             .hasPermission(scr, PluginManager::Permission::Operator)) {
        Chakra::ThrowError(
            util::StrToWStr(
                XOR_STRING("No permission to use getEntityList here")
            )
        );
        ...
        if (ent->getRuntimeID() == 1) {
            JS::JsSetIndexedProperty(
                ...
                lplrc->construct(
                    new JsEntity(
                        ent->getRuntimeID(),
                        JsEntity::AccessLevel::LocalPlayer
                    ),
                    true
                )
            );
            ...
            JS::JsSetIndexedProperty(
                array,
                db,
                plrc->construct(new JsEntity(ent->getRuntimeID()), true)
            );
        } else
            JS::JsSetIndexedProperty(
                array,
                db,
                entc->construct(new JsEntity(ent->getRuntimeID()), true)
            );
        idx++;
        Chakra::Release(db);
    }
...
JsValueRef GameScriptingObject::worldGetEntCount(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::dimensionGetName(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::dimensionGetBlock(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 4))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsValueType::JsNumber},
    ...
    JsScript* scr = JsScript::getThis();
    if (!Latite::getPluginManager().hasPermission(
            scr->getPlugin(),
            PluginManager::Permission::Operator
        )) {
        Chakra::ThrowError(
            util::StrToWStr(XOR_STRING("No permission to use getBlock here"))
        );
        return JS_INVALID_REFERENCE;
    }
...
JsValueRef GameScriptingObject::getMousePosCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::getScreenSizeCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    return JsScript::getThis()->getClass<JsVec2>()->construct(
        SDK::ClientInstance::get()->getGuiData()->screenSize
    );
}

JsValueRef GameScriptingObject::getInputBinding(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    auto key = Latite::getKeyboard().getMappedKey(
        util::WStrToStr(Chakra::GetString(arguments[1]))
    );
    return Chakra::MakeInt(key);
}

JsValueRef GameScriptingObject::getWorldCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::getDimensionCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::isInUICallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    return SDK::ClientInstance::get()->minecraftGame->isCursorGrabbed()
        ? Chakra::GetFalse()
        : Chakra::GetTrue();
}

JsValueRef GameScriptingObject::getFOV(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    auto lrp =
        SDK::ClientInstance::get()->levelRenderer->getLevelRendererPlayer();
    return JsScript::getThis()->getClass<JsVec2>()->construct(
        Vec2(lrp->getFovX(), lrp->getFovY())
    );
}

JsValueRef GameScriptingObject::getCameraPosition(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::captureCursor(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::releaseCursor(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::isKeyDown(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    return Latite::getKeyboard().isKeyDown(vKey) ? Chakra::GetTrue()
                                                 : Chakra::GetFalse();
}

JsValueRef GameScriptingObject::sendChatCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (Latite::getPluginManager()
            .hasPermission(script, PluginManager::Permission::SendChat)
    ...
    Chakra::ThrowError(
        util::StrToWStr(XOR_STRING("Permission denied to use sendChatMessage"))
    );
    return Chakra::GetUndefined();
}

JsValueRef GameScriptingObject::executeCommand(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (lp) {
        auto pkt =
            SDK::MinecraftPackets::createPacket(SDK::PacketID::COMMAND_REQUEST);
        SDK::CommandRequestPacket* cmd =
            reinterpret_cast<SDK::CommandRequestPacket*>(pkt.get());
...
JsValueRef GameScriptingObject::playSoundUI(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 4))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString},
             {arguments[2], JsNumber},
...
JsValueRef GameScriptingObject::getLocalPlayerCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    return cl->construct(
        new JsEntity(1, JsEntity::AccessLevel::LocalPlayer),
        true
    );
}

JsValueRef GameScriptingObject::getConnectedServerCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::getConnectedFeaturedServerCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef GameScriptingObject::getPortCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
