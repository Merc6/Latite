...
JsValueRef JsPlayerClass::playerGetName(
    JsValueRef callee,
    bool isConstructor,
...
JsValueRef JsPlayerClass::playerGetXboxUserID(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (ent && ent->validate() && ent->getEntity()->isPlayer()) {
        return Chakra::MakeString(
            util::StrToWStr(
                static_cast<SDK::Player*>(ent->getEntity())->getXUID()
            )
        );
    }
    Chakra::ThrowError(XW("Invalid player"));
...
JsValueRef JsPlayerClass::playerGetSelectedItem(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (ent && ent->validate() && ent->getEntity()->isPlayer()) {
        if (ent->level != JsEntity::AccessLevel::Restricted) {
            ...
            return JsScript::getThis()->getClass<JsItemStack>()->construct(
                supp->inventory->getItem(supp->selectedSlot),
                false
            );
        } else {
...
JsValueRef JsPlayerClass::playerGetSelectedSlot(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsPlayerClass::playerGetInventorySlot(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (ent && ent->validate() && ent->getEntity()->isPlayer()) {
        if (ent->level != JsEntity::AccessLevel::Restricted) {
            ...
            return JsScript::getThis()->getClass<JsItemStack>()->construct(
                supp->inventory->getItem(Chakra::GetInt(arguments[1])),
                false
            );
        } else {
