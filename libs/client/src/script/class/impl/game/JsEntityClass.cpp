...
JsValueRef JsEntityClass::entityIsValid(
    JsValueRef callee,
    bool isConstructor,
...
JsValueRef JsEntityClass::entityGetPos(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsEntityClass::entityGetRot(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsEntityClass::entityGetDimensionName(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsEntityClass::entityGetHurtTime(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsEntityClass::entityIsPlayer(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsEntityClass::entityAttack(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    JS::JsGetExternalData(arguments[0], reinterpret_cast<void**>(&ent));
    if (ent && ent->validate()
        && ent->getEntity() != SDK::ClientInstance::get()->getLocalPlayer()
        && SDK::ClientInstance::get()
                ->getLocalPlayer()
                ->getCommandPermissionLevel()
            > 1) {
...
JsValueRef JsEntityClass::entityGetType(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsEntityClass::entityIsLocalPlayer(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsEntityClass::entityGetHealth(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (ent && ent->validate()) {
        if (ent->level != JsEntity::AccessLevel::Restricted) {
            return Chakra::MakeDouble(
                static_cast<float>(ent->getEntity()->getHealth())
            );
        } else {
...
JsValueRef JsEntityClass::entityGetHunger(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (ent && ent->validate()) {
        if (ent->level != JsEntity::AccessLevel::Restricted) {
            return Chakra::MakeDouble(
                static_cast<float>(ent->getEntity()->getHunger())
            );
        } else {
...
JsValueRef JsEntityClass::entityGetSaturation(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (ent->level != JsEntity::AccessLevel::Restricted) {
        return Chakra::MakeDouble(
            static_cast<float>(ent->getEntity()->getSaturation())
        );
    } else {
...
JsValueRef JsEntityClass::entityGetVariable(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
        ...
        for (auto& var : ent->getEntity()->molangVariableMap.mVariables) {
            if (var->mName.getString().starts_with(XOR_STRING("variable."))) {
                if (var->mName == str) {
                    ...
                    Chakra::SetProperty(
                        obj,
                        util::StrToWStr(XOR_STRING("number")),
                        Chakra::MakeDouble(var->mValue.mPOD.mFloat)
                    );
                    return obj;
                }
            }
...
JsValueRef JsEntityClass::entitySetVariable(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString}, {arguments[2], JsNumber}}
        ))
        return JS_INVALID_REFERENCE;
...
JsValueRef JsEntityClass::entityGetPosInterpolated(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (ent && ent->validate()) {
        ...
        if (ent->level != JsEntity::AccessLevel::Restricted
            ...
            Vec3 interPos = {
                std::lerp(posOld.x, pos.x, alpha),
                std::lerp(posOld.y, pos.y, alpha),
                std::lerp(posOld.z, pos.z, alpha)
            };
            return cl->construct(interPos);
        } else {
            Chakra::ThrowError(
                XW("Access denied, cannot use getPositionInterpolated")
            );
            return Chakra::GetUndefined();
        }
    }
...
JsValueRef JsEntityClass::entityGetPosPrev(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (ent && ent->validate()) {
        ...
        if (ent->level != JsEntity::AccessLevel::Restricted
            ...
            Chakra::ThrowError(
                XW("Access denied, cannot use getPreviousPosition")
            );
            return Chakra::GetUndefined();
        }
    }
...
JsValueRef JsEntityClass::entityGetStatusFlag(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (ent && ent->validate()) {
        auto actor = ent->getEntity();
        return actor->getStatusFlag(Chakra::GetInt(arguments[1]))
            ? Chakra::GetTrue()
            : Chakra::GetFalse();
    }
    Chakra::ThrowError(XW("Invalid entity"));
...
JsValueRef JsEntityClass::entitySetStatusFlag(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 3))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsNumber}, {arguments[2], JsBoolean}}
        ))
        return JS_INVALID_REFERENCE;
    ...
    if (ent && ent->validate()
        ...
        auto actor = ent->getEntity();
        actor->setStatusFlag(
            Chakra::GetInt(arguments[1]),
            Chakra::GetBool(arguments[2])
        );
        return Chakra::GetUndefined();
    } else {
...
JsValueRef JsEntityClass::entityGetArmorSlot(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    return JsScript::getThis()->getClass<JsItemStack>()->construct(
        ent->getEntity()->getArmor(slot),
        false
    );
}

JsValueRef JsEntityClass::entitySetVelocity(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsEntityClass::entityGetVelocity(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
