...
JsValueRef JsLocalPlayerClass::getBreakProgress(
    JsValueRef callee,
    bool isConstructor,
...
JsValueRef JsLocalPlayerClass::getLastBreakProgress(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsLocalPlayerClass::getLookingAt(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    return Chakra::MakeInt((int)SDK::ClientInstance::get()
                               ->minecraft->getLevel()
                               ->getHitResult()
                               ->hitType);
}

JsValueRef JsLocalPlayerClass::getSelectedBlock(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    return JsScript::getThis()->getClass<JsVec3>()->construct(
        Vec3 {
            static_cast<float>(hr->hitBlock.x),
            static_cast<float>(hr->hitBlock.y),
            static_cast<float>(hr->hitBlock.z)
        }
    );
}

JsValueRef JsLocalPlayerClass::setMovementState(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsLocalPlayerClass::getMovementState(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
