...
bool JsScript::load() {
    auto res =
        JS::JsCreateContext(plugin->getRuntime(), &this->ctx) == JsNoError;
    if (!res)
        return false;
...
JsErrorCode JsScript::runScript() {
    ...
    try {
        code = JS::JsRunScript(
            loadedScript.c_str(),
            util::fnv1a_32(util::WStrToStr(this->path.wstring())),
...
JsErrorCode JsScript::compileScript() {
    ...
    auto err =
        JS::JsSerializeScript(loadedScript.c_str(), scriptBuf, &bufferSize);

    auto objFolder = this->getPlugin()->getPath() / "obj";
    std::filesystem::create_directory(objFolder);

    auto objPath = objFolder
        / (this->relPath.substr(0, relPath.size() - 3) /*remove .js*/ + L".obj"
        );
...
void JsScript::loadJSApi() {
    Chakra::SetContext(ctx);
    JsValueRef res;
    auto err = JS::JsRunScript(
        util::StrToWStr(Latite::get().getTextAsset(JS_LATITEAPI)).c_str(),
        sCtx,
        L"latiteapi.js",
        &res
    );
...
namespace {
JsValueRef CALLBACK setTimeoutCallback(
    JsValueRef callee,
    bool isConstructor,
    ...
    if (!Chakra::VerifyArgCount(argCount, 3))
        return Chakra::GetUndefined();
    auto sz = Chakra::VerifyParameters(
        {{arguments[1], JsValueType::JsFunction},
    ...
    auto tim = std::make_shared<JsScript::JsTimeout>(
        static_cast<int>(thi->timeouts.size() + 1),
...
JsValueRef CALLBACK setIntervalCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    JsScript* thi = reinterpret_cast<JsScript*>(callbackState);
    auto tim = std::make_shared<JsScript::JsTimeout>(
        static_cast<int>(thi->intervals.size() + 1),
        static_cast<long long>(num),
        func
    );
    thi->timeouts.push_back(tim);
...
JsValueRef CALLBACK clearTimeoutCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    std::erase_if(
        thi->intervals,
        [&](std::shared_ptr<JsScript::JsTimeout>& obj) -> bool {
...
JsValueRef CALLBACK clearIntervalCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef CALLBACK getEnvCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef CALLBACK sleepCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(
            static_cast<long long>(Chakra::GetNumber(arguments[1]))
        )
    );
    return Chakra::GetUndefined();
}

JsValueRef CALLBACK clientMessageCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef CALLBACK loadModule(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!mod) {
        ...
        Logger::Warn(
            "Terminating plugin {}! This is most likely caused by a circular dependency ({})->({})",
            util::WStrToStr(thi->getPlugin()->getName()),
            thi->getRelativePath().string(),
...
winrt::Windows::Foundation::IAsyncAction doRequestPermission(
    JsPlugin* plugin,
    ...
    winrt::Windows::UI::Popups::MessageDialog dialog(
        str,
        L"Permission Request"
    );
    ...
    Latite::getPluginManager().forEach(
        [plugin, &hasPlugin](std::shared_ptr<JsPlugin> plug) {
            if (plug.get() == plugin) {
                hasPlugin = true;
            }
        }
...
JsValueRef CALLBACK requestPermission(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    static std::wstring permSysAccess =
        util::StrToWStr(XOR_STRING("permission.system_access"));
    static std::wstring sysAccessMessage = util::StrToWStr(XOR_STRING(
        "This script is requesting the System Access permission.\n\nPlease make sure you trust this plugin!\nThis permission will give the plugin access to system functions, like playing system sounds, ejecting the CD drive, and many other things."
    ));
...
JsValueRef JsScript::AsyncOperation::call() {
    ...
    Latite::getPluginManager().handleErrors(
        Chakra::CallFunction(
            this->callback,
            this->args.data(),
            static_cast<unsigned short>(this->args.size()),
            &obj
        )
    );
    return obj;
