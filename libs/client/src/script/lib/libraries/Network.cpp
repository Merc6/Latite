...
void Network::deserializeHeaders(
    ...
    if (jsArray != JS_INVALID_REFERENCE && type == JsArray) {
        for (int i = 0; i < Chakra::GetIntProperty(jsArray, L"length"); i++) {
            ...
            if (type == JsObject) {
                std::wstring name =
                    Chakra::GetStringProperty(jsArrayItem, L"name");
                std::wstring value =
                    Chakra::GetStringProperty(jsArrayItem, L"value");
...
JsValueRef Network::get(
    ...
    auto op = std::make_shared<NetAsyncOperation>(
        arguments[3],
...
JsValueRef Network::getSync(
    ...
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString},
             {Chakra::TryGet(arguments, argCount, 2), JsObject, true}}
        ))
        return ret;
    auto ws = Chakra::GetString(arguments[1]);
...
JsValueRef Network::postSync(
    ...
    if (!Chakra::VerifyArgCount(argCount, 3))
        return ret;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString}, {arguments[2], JsObject}}
        ))
        return ret;
    auto ws = Chakra::GetString(arguments[1]);
