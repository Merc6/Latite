...
std::shared_ptr<JsPlugin>
PluginManager::loadPlugin(std::wstring const& folderPath, bool run) {
    ...
    for (auto& scr : this->items) {
        if (std::filesystem::absolute(scr->getPath())
            == std::filesystem::absolute(scriptPath)) {
            Latite::getClientMessageQueue().push(
                util::Format(
                    std::format(
                        "Plugin {} is already loaded.",
                        util::WStrToStr(scr->getName())
                    )
                )
            );
            return nullptr;
...
std::shared_ptr<JsPlugin>
PluginManager::getPluginByName(std::wstring const& name) {
...
void PluginManager::reportError(JsValueRef except, std::wstring filePath) {
    ...
    Logger::Info(
        "(plugin/{}) ({}) {}",
        util::WStrToStr(JsScript::getThis()->getPlugin()->getName()),
        JsScript::getThis()->getRelativePath().string(),
        util::WStrToStr(stack)
    );
    ...
    if (script) {
        if (code == JsErrorScriptException) {
            ...
            Latite::getClientMessageQueue().display(
                util::Format(
                    std::format(
                        "&cA JS error occured in script {}: JsErrorCode 0x{:X}",
                        util::WStrToStr(script->data.name),
                        (int)code
                    )
                )
            );
            Logger::Info(
                "(plugin/{}) ({}) Js ErrorCode: 0x{:X}",
                util::WStrToStr(script->getPlugin()->getName()),
                script->getRelativePath().string(),
                (int)code
            );
        }
    }
...
bool PluginManager::loadPrerunScripts() {
    if (!scriptingSupported()) {
        Logger::Warn(
            "Scripting is not supported. Please try restarting your game."
        );
        return false;
    }
    ...
    for (auto& dirEntry : std::filesystem::directory_iterator(prerunPath)) {
        if (dirEntry.is_directory()) {
            auto pluginJsonPath =
                dirEntry.path() / JsPlugin::PLUGIN_MANIFEST_FILE;
            std::ifstream ifs {pluginJsonPath};

            bool res = loadPlugin(dirEntry.path().filename().wstring(), true)
                == nullptr;
...
std::optional<int> PluginManager::installScript(std::string const& inName) {
    std::wstring registry = XW(
        "https://raw.githubusercontent.com/LatiteScripting/Scripts/master/Plugins"
    );
    ...
    auto message = [](std::string const& msg, bool err = false) -> void {
        if (err) {
            Latite::getClientMessageQueue().push(
                util::Format("[&5Plugin Manager&r] &c") + msg
            );
        } else
            Latite::getClientMessageQueue().push(
                util::Format("[&5Plugin Manager&r] ") + msg
            );
    };
    ...
    auto http = HttpClient();
    {
        ...
        try {
            ...
            if (response.IsSuccessStatusCode()) {
                try {
                    scriptsJson =
                        nlohmann::json::parse(std::wstring(strs.c_str()));
                } catch (nlohmann::json::parse_error& e) {
                    message(
                        "JSON error while installing plugin: "
                            + std::string(e.what()),
                        true
                    );
                    return 0;
                ...
                message(
                    "Could not fetch the plugin list. Are you connected to the internet?",
                    true
                );
                return 0;
            }
            ...
            Latite::getClientMessageQueue().push(
                util::WStrToStr(err.message().c_str())
            );
            return 0;
        }
    }
    ...
    for (auto& js : arr) {
        ...
        if (in == name) {
            ...
            for (auto& fil : js["files"]) {
                ...
                if (ofs.fail()) {
                    message(
                        "Error opening file: " + std::to_string(*_errno()),
                        true
                    );
                    return *_errno();
                }
        ...
        if (ev.second.size() > 0) {
            ...
            while (it != ev.second.end()) {
                ...
                auto removeIt = std::remove_if(
                    ...
                    [&](const auto& script) {
                        return !script
                            || std::get<2>(*it) == script->getContext();
                    }
                );
...
bool PluginManager::hasPermission(JsPlugin* script, Permission perm) {
...
#if LATITE_DEBUG
        Logger::Warn(
            "[Script] attempt to call hasPermission while player does not exist!"
        );
#endif
        return false;
    }
    switch (perm) {
        case Permission::SendChat:
            return script->isTrusted()
                || player->getCommandPermissionLevel() > 0;
    ...
    for (auto& lis : eventListeners) {
        if (lis.first == ev.type) {
            for (auto& l : lis.second) {
                ...
                for (auto& val : ev.values) {
                    ...
                    switch (val.val.index()) {
                        ...
                        case Event::Value::Number: {
                            JS::JsDoubleToNumber(
                                std::get<double>(val.val),
                                &ref
                            );
                        } break;
                        case Event::Value::String: {
                            JS::JsPointerToString(
                                std::get<std::wstring>(val.val).c_str(),
                                std::get<std::wstring>(val.val).size(),
                                &ref
                            );
                        } break;
                ...
                handleErrors(
                    Chakra::CallFunction(std::get<1>(l), params, 2, &ret)
                );
