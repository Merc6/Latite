...
bool CommandManager::runCommand(std::string const& line) {
    ...
    std::sort(
        items.begin(),
        items.end(),
        [](std::shared_ptr<ICommand>& left, std::shared_ptr<ICommand>& right) {
                    ...
                    try {
                        ...
                        if (!result) {
                            ...
                            Latite::getClientMessageQueue().push(
                                util::Format("&cUsage: " + usage)
                            );
                        }
                        return result;
                    } catch (std::exception& e) {
                        Logger::Warn(
                            "An unhandled exception occured while running this command: {}",
                            e.what()
                        );
                        Latite::getClientMessageQueue().push(
                            util::Format(
                                std::string(
                                    "&cAn unhandled exception occured while running this command: "
                                )
                                + e.what()
                            )
                        );
    ...
    Latite::getClientMessageQueue().push(
        util::Format(
            "&cUnknown command: " + (newArgs.empty() ? "" : newArgs[0]) + "."
        )
    );
    return false;
}
