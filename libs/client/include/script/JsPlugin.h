...
class JsPlugin final {
  public:
    ...
    [[nodiscard]]
    std::filesystem::path getMainIndexPath() {
        return mainScript->getPath();
    }
    ...
    inline static constexpr std::wstring_view PLUGIN_MANIFEST_FILE =
        L"plugin.json";
    ...
    loadOrFindModule(JsScript* script, std::wstring name);

    [[nodiscard]]
    std::wstring getCertificate();
    [[nodiscard]]
    static std::optional<std::wstring>
    getHash(std::filesystem::path const& main);

    std::vector<std::shared_ptr<JsScript>>& getScripts() {
        return scripts;
    }
    ...
    enum class UserPermission { SYSTEM_ACCESS };
