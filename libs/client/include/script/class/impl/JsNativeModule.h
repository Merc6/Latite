...
class JsNativeModule: public JsWrapperClass<void> {
  ...
  public:
    ...
    static JsValueRef CALLBACK getCallback(
        ...
        if (!JsScript::getThis()->getPlugin()->hasPermission(
                JsPlugin::UserPermission::SYSTEM_ACCESS
            )) {
    ...
    static JsValueRef CALLBACK callCallback(
        JsValueRef callee,
        bool isConstructor,
        JsValueRef* arguments,
        unsigned short argCount,
        ...
        if (!Chakra::VerifyParameters(
                {{arguments[1], JsString}, {arguments[2], JsString}}
            ))
            return JS_INVALID_REFERENCE;

        if (argCount > 18) {
            Chakra::ThrowError(
                XW("NativeModule.call can only accept 15 native arguments.")
            );
            return JS_INVALID_REFERENCE;
        }
...
#ifdef LATITE_DEBUG
                Chakra::ThrowError(
                    util::StrToWStr(XOR_STRING("You cannot use this function."))
                );
#endif
                return JS_INVALID_REFERENCE;
            }
        ...
        if (type == XW("int64")) {
            // int64_t
            auto res = ((any_func_int64_t)proc)(
                args[0],
                args[1],
                args[2],
                args[3],
            ...
            // int
            auto res = ((any_func_int32_t)proc)(
                args[0],
                args[1],
                args[2],
                args[3],
            ...
            // float
            auto res = ((any_func_float32_t)proc)(
                args[0],
                args[1],
                args[2],
                args[3],
            ...
            // double
            auto res = ((any_func_float64_t)proc)(
                args[0],
                args[1],
                args[2],
                args[3],
