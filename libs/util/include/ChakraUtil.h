...
#define FUNC(name) \
    using _Type_##name = decltype(&name); \
    inline static _Type_##name name = (_Type_##name)Chakra::pass(#name)
...
class Chakra {
  public:
    ...
    static JsErrorCode CallFunction(
        JsValueRef func,
    ...
    static JsValueRef
    TryGet(JsValueRef* args, unsigned short count, unsigned short idx);

    static void DefineFunc(
        JsValueRef obj,
    ...
    static Result VerifyParameters(
        std::initializer_list<ParamContainer> params,
        bool autoThrow = true
    );
    static Result VerifyArgCount(
        unsigned short has,
