...
class DrawUtil {
  ...
  public:
    enum class RenderType { D2D, Game } renderType = RenderType::D2D;

    enum class OutlinePosition : int { Center, Inside, Outside };
    ...
    virtual void drawRoundedRectangle(
        ...
        float radius = 10.f,
        float lineThickness = 1.f,
        OutlinePosition outPos = OutlinePosition::Center
    ) = 0;
    ...
    virtual Vec2 getTextSize(
        std::wstring const& text,
        Renderer::FontSelection font,
        float size,
        bool trailingWhitespace = true,
        bool cache = true,
        std::optional<Vec2> bounds = std::nullopt
    ) = 0;
    /// Be careful when setting cache to true
    virtual RectF getTextRect(
        std::wstring const& text,
        Renderer::FontSelection font,
        float size,
        float pad = 0.f,
...
class D2DUtil final: public DrawUtil {
  public:
    ...
    virtual void
    fillRectangle(RectF const& rect, d2d::Color const& color) override;
    virtual void drawRectangle(
        RectF const& rect,
        d2d::Color const& color,
        float lineThickness = 1.f
...
class MCDrawUtil final: public DrawUtil {
  public:
    ...
    MCDrawUtil(SDK::MinecraftUIRenderContext* ctx, SDK::Font* font) :
        renderCtx(ctx),
        scn(renderCtx->screenContext),
        font(font),
        guiScale(SDK::ClientInstance::get()->getGuiData()->guiScaleFrac) {}
