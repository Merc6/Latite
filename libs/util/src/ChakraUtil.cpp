...
FARPROC Chakra::pass(const char* name) {
    if (GetModuleHandleA(0) != GetModuleHandleA("Minecraft.Windows.exe"))
        return 0;
    ...
    if (!mod) {
        ...
        if (!mod) {
            ...
            mod = LoadLibraryW((util::GetLatitePath() / "Assets"
                                / "ChakraCore.dll")
                                   .wstring()
                                   .c_str());
        }
...
void Chakra::SetContext(JsContextRef context) {
    if (!Latite::isMainThread()) {
        Logger::Fatal(
            "{}",
            XOR_STRING("SetContext accessed outside of the main thread!!!")
        );
        __debugbreak();
    }
...
JsErrorCode Chakra::CallFunction(
    JsValueRef func,
...
JsValueRef
Chakra::TryGet(JsValueRef* args, unsigned short count, unsigned short idx) {
    if (idx >= count)
        return JS_INVALID_REFERENCE;
    return args[idx];
}

void Chakra::DefineFunc(
    JsValueRef obj,
...
Chakra::Result Chakra::VerifyParameters(
    std::initializer_list<ParamContainer> params,
    bool autoThrow
) {
    size_t count = 0;
    for (auto const& param : params) {
        count++;
        if (param.isOptional && param.val == JS_INVALID_REFERENCE)
            continue;
        JsValueType trueType;
        JS::JsGetValueType(param.val, &trueType);
        if (param.isOptional
            && (param.val == JS_INVALID_REFERENCE || trueType == JsUndefined
                || trueType == JsNull))
            continue;

        if (trueType != param.type) {
            std::wstring wstr = L"Parameter " + std::to_wstring(count)
                + L" must be of correct type (has "
                + Chakra::GetTypeName(trueType) + L", needs "
                + Chakra::GetTypeName(param.type) + L")";
            if (autoThrow) {
                ThrowError(wstr);
            }
...
Chakra::Result Chakra::VerifyArgCount(
    unsigned short has,
    ...
    auto ws =
        L"Function does not take " + std::to_wstring(has - 1) + L" arguments";
    if (autoThrow)
        Chakra::ThrowError(ws);
    return {false, ws};
}
