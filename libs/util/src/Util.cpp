...
std::filesystem::path util::GetRoamingPath() {
    ...
    if (!_wdupenv_s(&env, &size, L"localappdata") && env) {
        auto str =
            std::wstring(env).substr(0, lstrlenW(env) - 2) + L"RoamingState";
        delete env;
        return str;
    }
    return std::wstring();
...
std::wstring util::FormatWString(
    std::wstring const& formatString,
    std::vector<std::wstring> const& formatArgs
) {
    ...
    while (pos < formatString.length()) {
        if (formatString[pos] == L'{' && (pos + 1 < formatString.length())
            && formatString[pos + 1] == L'}') {
...
int util::StringToKey(std::string const& s) {
    ...
    for (auto& ent : KeyNames) {
        if (ToLower(ent.second) == ToLower(s))
            return static_cast<int>(ent.first);
    }
    return 0;
}
...
void util::PlaySoundUI(std::string const& sound, float volume, float pitch) {
    ...
    if (lr) {
        cInst->minecraft->getLevel()->playSoundEvent(
            sound,
            lr->getLevelRendererPlayer()->getOrigin(),
            volume,
            pitch
        );
...
Vec2 util::QuaternionToRot(const Vec4& quat) {
    ...
    if (std::abs(sinp) >= 1.f) {
        pitch =
            std::copysign(pi_f / 2.f, sinp); // use 90 degrees if out of range
    } else {
        pitch = std::asin(sinp);
    }
