...
HurtColor::HurtColor() :
    Module(
        "HurtColor",
        LocalizeString::get("client.module.hurtColor.name"),
        LocalizeString::get("client.module.hurtColor.desc"),
        GAME
    ) {
    addSetting(
        "color",
        LocalizeString::get("client.module.hurtColor.color.name"),
        L"",
        color
    );

    listen<OverlayColorEvent>(
        static_cast<EventListenerFunc>(&HurtColor::onActorOverlay)
    );
}
