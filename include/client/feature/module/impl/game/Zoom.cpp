...
Zoom::Zoom() :
    Module(
        "Zoom",
        LocalizeString::get("client.module.zoom.name"),
        LocalizeString::get("client.module.zoom.desc"),
        GAME,
        nokeybind
    ) {
    addSetting(
        "zoomKey",
        LocalizeString::get("client.module.zoom.zoomKey.name"),
        LocalizeString::get("client.module.zoom.zoomKey.desc"),
        this->zoomKey
    );
    addSliderSetting(
        "modifier",
        LocalizeString::get("client.module.zoom.modifier.name"),
        LocalizeString::get("client.module.zoom.modifier.desc"),
        this->modifier,
        FloatValue(1.f),
        FloatValue(50.f),
        FloatValue(1.f)
    ...
    addSetting(
        "animation",
        LocalizeString::get("client.module.zoom.animation.name"),
        LocalizeString::get("client.module.zoom.animation.desc"),
        hasAnim
    );
    addSliderSetting(
        "animationSpeed",
        LocalizeString::get("client.module.zoom.animationSpeed.name"),
        LocalizeString::get("client.module.zoom.animationSpeed.desc"),
        animSpeed,
        FloatValue(1.f),
        FloatValue(5.f),
        FloatValue(1.f),
    ...
    addSetting(
        "cinematic",
        LocalizeString::get("client.module.zoom.cinematic.name"),
        LocalizeString::get("client.module.zoom.cinematic.desc"),
        this->cinematicCam
    );
    addSetting(
        "hideHand",
        LocalizeString::get("client.module.zoom.hideHand.name"),
        LocalizeString::get("client.module.zoom.hideHand.desc"),
        this->hideHand
    );
    addSetting(
        "dpiAdjust",
        LocalizeString::get("client.module.zoom.dpiAdjust.name"),
        LocalizeString::get("client.module.zoom.dpiAdjust.desc"),
        this->dpiAdjust
    );
    ...
    listen<CinematicCameraEvent>(
        static_cast<EventListenerFunc>(&Zoom::onCinematicCamera)
    );
...
void Zoom::onRenderLevel(Event& evGeneric) {
    ...
    modifyTo = std::clamp(
        shouldZoom ? std::get<FloatValue>(modifier).value + zoomModifier : 1.f,
        1.f,
        60.f
    );
    ...
    if (std::get<BoolValue>(hasAnim))
        lr = std::lerp(
            activeModifier,
            modifyTo,
            alpha * std::get<FloatValue>(animSpeed) / 10.f
        );
    activeModifier = lr;
...
void Zoom::onSensitivity(Event& evG) {
    ...
    if (shouldZoom && std::get<BoolValue>(this->dpiAdjust)) {
        ev.getValue() = std::min(
            ev.getValue(),
            ev.getValue() * (2 / std::get<FloatValue>(modifier))
        );
    }
}