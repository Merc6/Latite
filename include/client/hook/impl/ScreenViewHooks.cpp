...
void __fastcall ScreenViewHooks::setupAndRender(
    SDK::ScreenView* view,
    void* ctx
) {
    ...
    RenderLayerEvent ev {
        view,
        reinterpret_cast<SDK::MinecraftUIRenderContext*>(ctx)
    };
...
ScreenViewHooks::ScreenViewHooks() : HookGroup("ScreenView") {
    setupAndRenderHook = addHook(
        Signatures::ScreenView_setupAndRender.result,
        setupAndRender,
