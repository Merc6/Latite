...
class IEventManager {
  public:
        ...
        requires std::derived_from<T, Event>
    {
        ...
        listeners.push_back(
            {T::hash,
             EventListener {listener, ptr, callWhileInactive, priority}}
        );
        std::sort(
            listeners.begin(),
            listeners.end(),
            [](std::pair<uint32_t, EventListener> const& left,
