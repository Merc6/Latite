...
struct ColorValue {
    ...
    ColorValue(nlohmann::json& js) {
        ...
        if (isChroma) {
            ...
            if (numColors >= 2)
                color2.get(js["color2"]);
            if (numColors >= 3)
                color3.get(js["color3"]);
        }
    }
...
struct SnapValue {
    enum Type : int { Normal, MCUI, Module };

    enum Pos : int { Right, Middle, Left };
...
class EnumData {
  ...
  public:
    ...
    [[nodiscard]]
    std::vector<EnumEntry>* getEntries() {
        return &entries;
    }
    ...
    [[nodiscard]]
    std::wstring getSelectedDesc() {
        return entries[std::get<EnumValue>(selectedIdx)].desc();
    }

    void next() {
        if (++std::get<EnumValue>(selectedIdx).val >= entries.size())
            std::get<EnumValue>(selectedIdx) = 0;
    }
...
class Setting: public Feature {
  public:
    struct Condition final {
        ...
        Condition(const char* name /*shut up c++*/) :
            type(EQUALS),
            settingName(name),
            values({1}) {}
    ...
    [[nodiscard]]
    bool shouldRender(class SettingGroup& group);

    std::wstring desc() override {
        return description;
    }
