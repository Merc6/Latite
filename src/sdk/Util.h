...
class Unknown {
  public:
    ...
    [[nodiscard]]
    inline void
    _do(std::function<void(Unknown<PossibleTypes...>)> func) noexcept {}
};
} // namespace SDK
...
extern TreatAs& directAccess(Pointer ptr, size_t offset) {
    return *reinterpret_cast<TreatAs*>(
...
#define CLASS_FIELD(type, name, offset) \
    __declspec(property(get = __get_field_##name, put = __set_field_##name) \
    ) type name; \
    type& __get_field_##name() const { \
        return util::directAccess<type>(this, offset); \
    } \
    template<typename T> \
    void __set_field_##name(const T& value) { \
        util::directAccess<type>(this, offset) = value; \
    }

#define MVCLASS_FIELD(type, name, ...) \
    __declspec(property(get = __get_field_##name, put = __set_field_##name) \
    ) type name; \
    type& __get_field_##name() const { \
        return util::directAccess<type>( \
            this, \
            SDK::mvGetOffset<__VA_ARGS__>() \
        ); \
    } \
    template<typename T> \
    void __set_field_##name(const T& value) { \
        util::directAccess<type>(this, (SDK::mvGetOffset<__VA_ARGS__>())) = \
            value; \
    }
