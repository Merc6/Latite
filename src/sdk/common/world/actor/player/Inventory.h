...
class Inventory { /*copied from bedrock server, not perfect*/
  public:
    ...
    virtual void
    addContentChangeListener(class ContainerContentChangeListener*) = 0;
    virtual void
    removeContentChangeListener(ContainerContentChangeListener*) = 0;
  ...
  private:
    ...
    virtual std::vector<const ItemStack*, std::allocator<const ItemStack*>>
    getSlots(void) = 0;
    ...
    virtual void createTransactionContext(
        std::function<
            void(Inventory&, int, ItemStack const&, ItemStack const&)>,
