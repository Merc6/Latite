...
struct TextMeasureData {
    ...
    constexpr TextMeasureData(
        float textSize,
        bool displayShadow,
        bool showColorSymbols
    ) :
        textSize(textSize),
        displayShadow(displayShadow),
        showColorSymbols(showColorSymbols) {}
};
...
class MinecraftUIRenderContext {
    ...
    void drawImage(
        ...
        if (internalVers >= V1_21_50) {
            ...
            memory::callVirtual<
                ...
                Vec2 const&,
                Vec2 const&,
                bool>(this, 7, texture, pos, size, uvPos, uvSize, false);
        } else {
            memory::callVirtual<
                void>(this, 7, texture, pos, size, uvPos, uvSize);
        }
    }

    virtual ~MinecraftUIRenderContext() = 0; // 0x0
    virtual void
    getLineLength(class Font*, std::string const&, float, bool) = 0; // 0x1
  ...
  public:
    ...
    virtual void
    flushImages(Color const&, float, HashedString const&) = 0; // 0x9
    virtual void beginSharedMeshBatch(void*) = 0; // 0xA
    virtual void endSharedMeshBatch(void*) = 0; // 0xB
    virtual void
    drawRectangle(RectangleArea const&, Color const&, float, int) = 0; // 0xC
    virtual void
    fillRectangle(RectangleArea const&, Color const&, float) = 0; // 0xD
    ...
    virtual TexturePtr*
    getTexture(TexturePtr* ptr, ResourceLocation const&, bool) = 0; // 0x1D
    virtual TexturePtr* getZippedTexture(
        TexturePtr* ptr,
        std::string const&,
