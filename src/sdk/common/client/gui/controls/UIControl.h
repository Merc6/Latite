...
class UIControl {
  ...
  public:
    std::vector<UIComponent*>
        uiComponents; //0x00B0 supposed to be vector of UIComponent
  private:
  ...
  public:
    ...
    void
    getControls(std::function<void(std::shared_ptr<UIControl> control)> func) {
        for (size_t i = 0; i < children.size(); i++) {
            auto child = children[i];
            func(child);
        }
    ...
    void getDescendants(
        std::function<void(std::shared_ptr<UIControl> control)> func
    ) {
        for (size_t i = 0; i < children.size(); i++) {
            auto child = children[i];
            func(child);
