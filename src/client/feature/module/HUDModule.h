...
class HUDModule: public Module {
  public:
    ...
    HUDModule(
        ...
        addSetting(
            "pos",
            LocalizeString::get("client.hudmodule.props.pos.name"),
            L"<internal setting>",
            storedPos
        );
        addSetting(
            "snapX",
            LocalizeString::get("client.hudmodule.props.snapX.name"),
            L"<internal setting>",
            snappingX
        );
        addSetting(
            "snapY",
            LocalizeString::get("client.hudmodule.props.snapY.name"),
            L"<internal setting>",
            snappingY
        );
        ...
        if (resizable)
            addSliderSetting(
                "scale",
                LocalizeString::get("client.hudmodule.props.size.name"),
                L"",
                scale,
                FloatValue(min_scale),
                FloatValue(max_scale),
                FloatValue(0.05f)
            );
        addSetting(
            "forceMinecraftRend",
            LocalizeString::get("client.hudmodule.props.forceMcRend.name"),
            LocalizeString::get("client.hudmodule.props.forceMcRend.desc"),
            forceMCRend
        );
    }
    ...
    [[nodiscard]]
    float getScale() {
        return std::get<FloatValue>(scale);
    }
    ...
    [[nodiscard]]
    d2d::Rect getRectNonScaled() {
        return rect;
    }

    [[nodiscard]]
    virtual bool forceMinecraftRenderer() override {
        return std::get<BoolValue>(forceMCRend);
    }
    ...
    void setScale(float f) {
        std::get<FloatValue>(scale) = f;
    }

    void setRect(d2d::Rect const& rc) {
        this->rect = rc;
    }
