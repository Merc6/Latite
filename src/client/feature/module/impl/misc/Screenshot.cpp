...
Screenshot::Screenshot() :
    Module(
        "Screenshot",
        L"Screenshot",
        L"Take a screenshot with a key.",
        GAME,
        nokeybind
    ) {
    ...
    listen<RenderOverlayEvent>(
        (EventListenerFunc)&Screenshot::onRenderOverlay,
        false,
        0 /*lowest priority so that Latite renders everything else*/
    );

    addSetting(
        "ssKey",
        L"Screenshot key",
        L"The key you press to take a screenshot",
        this->screenshotKey
    );
}
...
void Screenshot::onRenderOverlay(Event& ev) {
    ...
    if (queueToScreenshot) {
        ...
        Latite::getClientMessageQueue().push(
            std::format(
                "Screenshot saved to {}",
                (screenshotPath / "screenshot.png").string()
            )
        );
    ...
    if (savedBitmap) {
        ...
        if (std::chrono::duration_cast<std::chrono::milliseconds>(dist).count()
            > 4000ll /*4 seconds*/) {
        ...
        if (flashLerp > 0.f) {
            ...
            dc.fillRectangle(
                {0.f, 0.f, ss.width, ss.height},
                {1.f, 1.f, 1.f, flashLerp}
            );
        }
    }
}
...
Screenshot::takeScreenshot(std::filesystem::path const& path) {
    // file
    auto folder =
        co_await StorageFolder::GetFolderFromPathAsync(path.wstring());
    ...
    ThrowIfFailed(CreateStreamOverRandomAccessStream(
        raStream.as<IUnknown>().get(),
        IID_PPV_ARGS(&stream)
    ));
    ...
    ThrowIfFailed(
        wicBitmapEncoder->Initialize(stream.Get(), WICBitmapEncoderNoCache)
    );

    ComPtr<IWICBitmapFrameEncode> wicFrameEncode;
    ThrowIfFailed(wicBitmapEncoder->CreateNewFrame(
        &wicFrameEncode,
        nullptr // No encoder options.
    ));

    ThrowIfFailed(wicFrameEncode->Initialize(nullptr));

    ComPtr<IWICImageEncoder> imageEncoder;
    ThrowIfFailed(wicFactory->CreateImageEncoder(
        Latite::getRenderer().getDevice(),
        &imageEncoder
    ));

    ThrowIfFailed(imageEncoder->WriteFrame(
        bmp.Get(),
        wicFrameEncode.Get(),
        nullptr // Use default WICImageParameter options.
    ));

    ThrowIfFailed(wicFrameEncode->Commit());

    ThrowIfFailed(wicBitmapEncoder->Commit());

    // Flush all memory buffers to the next-level storage object.
    ThrowIfFailed(stream->Commit(STGC_DEFAULT));
