...
DebugInfo::DebugInfo() :
    Module(
        "DebugInfo",
        LocalizeString::get("client.module.debugInfo.name"),
        LocalizeString::get("client.module.debugInfo.desc"),
        GAME,
        VK_F3
    ) {
    listen<RenderLayerEvent>(
        static_cast<EventListenerFunc>(&DebugInfo::onRenderOverlay)
    );
    listen<DrawHUDModulesEvent>(
        static_cast<EventListenerFunc>(&DebugInfo::onRenderHUDModules),
        false,
        2
    );
}
...
std::string getDimension() {
    return std::format(
        "Dimension: {}",
        SDK::ClientInstance::get()->getLocalPlayer()->dimension->dimensionName
    );
}

std::string getCoordinates() {
    Vec3 position = SDK::ClientInstance::get()->getLocalPlayer()->getPos();
    std::string ln1 = std::format(
        "XYZ: {:.1f} / {:.1f} / {:.1f}",
        position.x,
        position.y,
        position.z
    ...
    std::string ln2;
    if (SDK::ClientInstance::get()->getLocalPlayer()->dimension->dimensionName
        == "Nether")
        ln2 = std::format(
            "Overworld: {:.1f} / {:.1f} / {:.1f}",
            position.x * 8.f,
...
std::string getMemUsage() {
    ...
    double totalMemoryGB = static_cast<double>(memStatus.ullTotalPhys)
        / (1024 * 1024 * 1024); // Convert to GB
...
std::string getGpuInfo() {
    return std::format(
        "Display: Unknown (DirectX{})",
        Latite::getRenderer().isDX11ByDefault() ? "11/10.1" : "12"
    );
}
...
void DebugInfo::onRenderOverlay(Event& evG) {
    ...
    if (ev.getScreenView()->visualTree->rootControl->name == "hud_screen") {
        ...
        const std::wstring topRightDebugInfo = util::StrToWStr(
            std::format(
                "{}\n{}\n{}\n",
                getMemUsage(),
                getGpuInfo(),
