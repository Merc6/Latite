...
TestModule::TestModule() :
    HUDModule(
        "TestModule",
        L"TestModule",
        L"Module for testing new things.",
        GAMEHUD
    ) {
...
void TestModule::render(DrawUtil& ctx, bool isDefault, bool inEditor) {
...

    {
        ...
        if (col.numColors == 2) {
            d2d::Color colors[2] = {
                d2d::Color(col.color1),
                d2d::Color(col.color2)
            };
            ...
            const D2D1_GRADIENT_STOP stops[] = {
                0.f,
                d2d::Color(col.color1).get(),
                0.5f,
                d2d::Color(col.color2).get(),
                1.f,
                d2d::Color(col.color3).get()
            };
            dc->CreateGradientStopCollection(
                stops,
                _countof(stops),
    ...
    dc.fillRectangle(
        {0.f, 0.f, rect.getWidth(), rect.getHeight()},
        brush.Get()
    );
...
void TestModule::onRender(Event& evG) {
    ...
    auto jpegCommentWriter = std::make_unique<JpegCommentWriter>(
        scn->tess,
        MaterialPtr::getUITextureAndColor()
    );
...
void TestModule::onRenderLayer(Event& evG) {
    ...
    if (ev.getScreenView()->visualTree->rootControl->name
        == "inventory_screen") {
        auto info = ((SDK::ContainerScreenController*)ev.getScreenView()
                         ->screenController)
                        ->_getSelectedSlotInfo();
        int test = 32;
    }
}
