...
Nickname::Nickname() :
    Module(
        "Nickname",
        LocalizeString::get("client.module.nickname.name"),
        LocalizeString::get("client.module.nickname.desc"),
        GAME
    ) {
    addSetting(
        "nick",
        LocalizeString::get("client.module.nickname.name"),
        LocalizeString::get("client.module.nickname.newNick.desc"),
        this->nickname
    );
...
void Nickname::onClientTextPacket(Event& evG) {
    ...
    std::string newName =
        util::WStrToStr(std::get<TextValue>(this->nickname).str);
    auto& currentPlayerName =
        SDK::ClientInstance::get()->getLocalPlayer()->playerName;
