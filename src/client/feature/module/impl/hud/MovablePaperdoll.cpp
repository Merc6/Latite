...
MovablePaperdoll::MovablePaperdoll() :
    HUDModule(
        "Paperdoll",
        LocalizeString::get("client.hudmodule.movablePaperdoll.name"),
        LocalizeString::get("client.hudmodule.movablePaperdoll.desc"),
        HUD
    ) {
    addSetting(
        "alwaysShow",
        LocalizeString::get(
            "client.hudmodule.movablePaperdoll.alwaysShow.name"
        ),
        LocalizeString::get(
            "client.hudmodule.movablePaperdoll.alwaysShow.desc"
        ),
        alwaysShow
    );

    listen<RenderLayerEvent>(
        static_cast<EventListenerFunc>(&MovablePaperdoll::onRenderLayer),
        true,
        ...
        if (ev.getScreenView()->visualTree->rootControl->name
            ...
            if (hudPlayer) {
                ...
                if (std::get<BoolValue>(alwaysShow)) {
                    auto comp =
                        (SDK::CustomRenderComponent*)hudPlayer->uiComponents[4];
