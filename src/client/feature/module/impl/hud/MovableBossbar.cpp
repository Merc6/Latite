...
MovableBossbar::MovableBossbar() :
    HUDModule(
        "MovableBossbar",
        LocalizeString::get("client.hudmodule.movableBossbar.name"),
        LocalizeString::get("client.hudmodule.movableBossbar.desc"),
        HUD,
        0,
        false
    ) {
    listen<RenderLayerEvent>(
        (EventListenerFunc)&MovableBossbar::onRenderLayer,
        true,
        10 /*need to overpower the hud renderer*/
    );
    addSetting(
        "hideBossbar",
        LocalizeString::get("client.hudmodule.movableBossbar.hideBossbar.name"),
        L"",
        this->hideBossbar
    );
}
        ...
        if (ev.getScreenView()->visualTree->rootControl->name
            ...
            if (bossHealthGrid) {
                if (std::get<BoolValue>(this->hideBossbar)) {
                    ...
                    this->rect.right =
                        rect.left + bossHealthGrid->bounds.x * guiScale;
                    this->rect.bottom =
                        rect.top + bossHealthGrid->bounds.y * guiScale;
                }
            } else {
