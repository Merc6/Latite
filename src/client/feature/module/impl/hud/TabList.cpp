...
TabList::TabList() :
    Module(
        "PlayerList",
        LocalizeString::get("client.module.tabList.name"),
        LocalizeString::get("client.module.tabList.desc"),
        HUD,
        VK_TAB
    ) {
    addSetting(
        "textColor",
        LocalizeString::get("client.module.tabList.textColor.name"),
        LocalizeString::get("client.module.tabList.textColor.desc"),
        textCol
    );
    addSetting(
        "bgColor",
        LocalizeString::get("client.module.tabList.bgColor.name"),
        LocalizeString::get("client.module.tabList.bgColor.desc"),
        bgCol
    );
    listen<RenderOverlayEvent>(
        static_cast<EventListenerFunc>(&TabList::onRenderOverlay)
    );
}
...
void TabList::onRenderOverlay(Event& evG) {
    ...
    std::wstring txt;
    if (SDK::RakNetConnector::get()
        && SDK::RakNetConnector::get()->featuredServer.size() > 0) {
    ...
    for (auto& ent : *lvl->getPlayerList()) {
        auto w = dc.getTextSize(util::StrToWStr(ent.second.name), font, textP).x
            + 3.f;
    ...
    size_t maxPerTab = 15;
    int numTabs = static_cast<int>(
        std::ceil(static_cast<float>(size) / static_cast<float>(maxPerTab))
    );
    ...
    float calcHeight = sectionHeight
        * ((static_cast<float>(size) > maxPerTab) ? maxPerTab
    ...
    dc.ctx->SetTransform(
        mat * D2D1::Matrix3x2F::Translation(ss.x / 2.f - calcWidth / 2.f, 20.f)
    );

    dc.fillRectangle(
        {0.f, 0.f, calcWidth, calcHeight + oY},
        std::get<ColorValue>(this->bgCol).getMainColor()
    );
    dc.drawRectangle(
        {0.f, 0.f, calcWidth, calcHeight + oY},
        d2d::Color(std::get<ColorValue>(this->bgCol).getMainColor())
            .asAlpha(1.f),
        2.f
    );

    dc.drawText(
        {0.f, 0.f, calcWidth, oY},
        txt,
        std::get<ColorValue>(this->textCol).getMainColor(),
        font,
        textP,
    ...
    for (auto& ent : *lvl->getPlayerList()) {
        ...
        for (auto& user : Latite::get().getLatiteUsers()) {
            if (user == ent.second.name) {
                ...
                dc.ctx->DrawBitmap(
                    Latite::getAssets().logoWhite.getBitmap(),
                    logoRc
                );
            }
        }
        ...
        dc.drawText(
            rc,
            name,
            std::get<ColorValue>(textCol).getMainColor(),
            font,
            textP,
