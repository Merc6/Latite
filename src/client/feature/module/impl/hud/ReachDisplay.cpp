...
ReachDisplay::ReachDisplay() :
    TextModule(
        "ReachDisplay",
        LocalizeString::get("client.textmodule.reachDisplay.name"),
        LocalizeString::get("client.textmodule.reachDisplay.desc"),
        HUD
    ) {
    ...
    addSliderSetting(
        "decimals",
        LocalizeString::get("client.textmodule.reachDisplay.decimals.name"),
        LocalizeString::get("client.textmodule.reachDisplay.decimals.desc"),
        this->decimals,
        FloatValue(0.f),
        FloatValue(6.f),
        FloatValue(1.f)
...
std::wstringstream ReachDisplay::text(bool isDefault, bool inEditor) {
    ...
    return (std::wstringstream()
            << std::fixed
            << std::setprecision((size_t)(std::get<FloatValue>(decimals))))
        << reach;
}
