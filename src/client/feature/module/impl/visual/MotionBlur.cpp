...
MotionBlur::MotionBlur() :
    Module(
        "MotionBlur",
        LocalizeString::get("client.module.motionBlur.name"),
        LocalizeString::get("client.module.motionBlur.desc"),
        GAME
    ) {
    addSliderSetting(
        "intensity",
        LocalizeString::get("client.module.motionBlur.intensity.name"),
        L"",
        intensity,
        FloatValue(0.f),
        FloatValue(10.f),
    ...
    addSliderSetting(
        "bleed",
        LocalizeString::get("client.module.motionBlur.bleed.name"),
        L"",
        antiBleed,
        FloatValue(0.f),
        FloatValue(1.f),
            ...
            for (auto& map : motionBlurList) {
                if (map) {
                    ...
                    ctx->DrawBitmap(
                        map,
                        &rc,
                        (std::get<FloatValue>(intensity) / 10.f) - 0.05f
                    );
                    ctx->DrawBitmap(
                        grainOfSalt,
                        &rc,
                        std::get<FloatValue>(antiBleed)
                    );
                }
            }
            ctx->Flush();
