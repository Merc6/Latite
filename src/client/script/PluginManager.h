...
class PluginManager final: public Listener, public Manager<class JsPlugin> {
  ...
  public:
    ...
    std::shared_ptr<class JsPlugin>
    loadPlugin(std::wstring const& folderPath, bool run = true);
    ...
    struct Event {
        ...
        struct Value {
            enum ValueType { Number = 1, String, EntityRef, Bool };

            std::wstring name;
            std::variant<std::nullptr_t, double, std::wstring, int64_t, bool>
                val = nullptr;
    ...
    std::unordered_map<
        std::wstring,
        std::vector<std::tuple<int, JsValueRef, JsContextRef>>>
        eventListeners;
