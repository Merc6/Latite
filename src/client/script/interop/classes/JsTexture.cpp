...
JsTexture::JsTexture(std::wstring const& textureNameOrPath, bool gameTexture) {
    ...
    Latite::get().queueForDXRender([this, path](ID2D1DeviceContext* ctx) {
        ...
        if (FAILED(ctx->CreateBitmapFromWicBitmap(
                conv.Get(),
                NULL,
                this->d2dTexture.GetAddressOf()
            ))) {
            this->failed = true;
        }
    });
...
void JsTexture::loadMinecraft() {
    ...
    if (gameTexture) {
        ...
        Latite::get().queueForUIRender(
            [this](SDK::MinecraftUIRenderContext* ctx) {
                ctx->getTexture(
                    &this->mcTexture.value(),
                    SDK::ResourceLocation(
                        util::WStrToStr(nameOrPath),
                        0 /*0 = default minecraft texture*/
                    ),
                    false /*not external*/
                );
            }
    ...
    // FIXME: I sure hope this object doesn't get destroyed by the time this function calls
    Latite::get().queueForUIRender([this,
                                    path](SDK::MinecraftUIRenderContext* ctx) {
        ctx->getTexture(
            &this->mcTexture.value(),
            SDK::ResourceLocation(
                util::WStrToStr(path),
                2 /*2 = external texture*/
            ),
            true /*external*/
        );
    });
...
std::filesystem::path JsTexture::tryGetRealPath(std::wstring const& oPath) {
    auto scriptPath =
        std::filesystem::path(JsScript::getThis()->getFolderPath());
    if (std::filesystem::exists(scriptPath / oPath))
        return scriptPath / oPath;
    return oPath;
}
