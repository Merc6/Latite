...
JsValueRef JsSettingClass::getValueCallback(
    JsValueRef callee,
    bool isConstructor,
    ...
    switch ((Setting::Type)set->value->index()) {
        case Setting::Type::Bool:
            return std::get<BoolValue>(*set->value) ? Chakra::GetTrue()
                                                    : Chakra::GetFalse();
        ...
        case Setting::Type::Float:
            return Chakra::MakeDouble(
                static_cast<double>(std::get<FloatValue>(*set->value).value)
            );
        ...
        case Setting::Type::Color: {
            ...
            if (colClass) {
                return colClass->construct(
                    d2d::Color(std::get<ColorValue>(*set->value).getMainColor())
                );
            }
            break;
        }
        case Setting::Type::Vec2: {
            ...
            if (colClass) {
                return colClass->construct(Vec2(
                    std::get<Vec2Value>(*set->value).x,
                    std::get<Vec2Value>(*set->value).y
                ));
            }
            break;
        }
...
JsValueRef JsSettingClass::setValueCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    switch ((Setting::Type)set->value->index()) {
        ...
        case Setting::Type::Float:
            if (argType == JsNumber) {
                std::get<FloatValue>(*set->value).value =
                    Chakra::GetNumber(setVal);
            }
            return undef;
        case Setting::Type::Text:
            if (argType == JsString) {
                std::get<TextValue>(*set->value).str =
                    Chakra::GetString(setVal);
            }
            return undef;
        ...
        case Setting::Type::Color:
            if (argType == JsObject) {
                ...
                std::get<ColorValue>(*set->value).getMainColor() =
                    StoredColor(col.r, col.g, col.b, col.a);
            }
            return undef;
        case Setting::Type::Enum:
            if (argType == JsNumber) {
...
JsValueRef JsSettingClass::setCondition(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 2, true, true))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString},
             {Chakra::TryGet(arguments, argCount, 2), JsBoolean, true}}
        ))
        return JS_INVALID_REFERENCE;
    ...
    set->condition = Setting::Condition(
        ...
        Setting::Condition::EQUALS,
        {static_cast<int>(setBool)}
    );
    return Chakra::GetUndefined();
}
