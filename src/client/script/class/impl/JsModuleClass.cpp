...
JsValueRef JsModuleClass::moduleIsEnabled(
    JsValueRef callee,
    bool isConstructor,
...
JsValueRef JsModuleClass::moduleSetEnabled(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyParameters({{arguments[1], JsBoolean}}))
...
JsValueRef JsModuleClass::moduleIsBlocked(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsModuleClass::moduleSetOnEvent(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString}, {arguments[2], JsFunction}}
        ))
        return JS_INVALID_REFERENCE;
    ...
    if (mod->isHud())
        hMod = reinterpret_cast<JsHUDModule*>(mod);
    if (mod->isTextual())
        tMod = reinterpret_cast<JsTextModule*>(mod);
    ...
    if (tMod) {
        ...
        return arguments[0];
    } else if (mod->eventListeners.find(str) != mod->eventListeners.end()) {
        JsContextRef ctx;
        JS::JsGetCurrentContext(&ctx);
        JS::JsAddRef(arguments[2], nullptr);
...
JsValueRef JsModuleClass::moduleGetSettings(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (mod) {
        JsValueRef array;
        JS::JsCreateArray(
            static_cast<unsigned int>(mod->settings->size()),
            &array
        );
        ...
        mod->settings->forEach([&](std::shared_ptr<Setting> set) {
            ...
            JS::JsSetIndexedProperty(
                array,
                index,
                setClass->construct(set.get(), false)
            );
...
JsValueRef JsModuleClass::moduleAddBoolSetting(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString},
             ...
             {arguments[3], JsString},
             {arguments[4], JsBoolean}}
        ))
        return JS_INVALID_REFERENCE;
    ...
    *set->value = BoolValue(Chakra::GetBool(arguments[4]));
    ...
    return setClass->construct(
        set.get(),
        false /*do not destroy the setting once it goes out of scope, as module manager will handle that*/
    );
}

JsValueRef JsModuleClass::moduleAddNumberSetting(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 8))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString},
             {arguments[2], JsString},
    ...
    *set->value =
        FloatValue(static_cast<float>(Chakra::GetNumber(arguments[7])));
...
JsValueRef JsModuleClass::moduleAddKeySetting(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsModuleClass::moduleAddTextSetting(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsModuleClass::moduleAddColorSetting(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef JsModuleClass::moduleAddEnumSetting(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
