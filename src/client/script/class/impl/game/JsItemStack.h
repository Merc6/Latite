  ...
  protected:
    static JsValueRef CALLBACK toStringCallback(
        ...
        std::string add = std::format(
            ...
            item->item ? item->getHoverName() : "air",
            item->aux
        );
        return Chakra::MakeString(L"[object " + util::StrToWStr(add) + L"]");
    }

    static JsValueRef CALLBACK getDisplayNameCallback(
        JsValueRef callee,
        bool isConstructor,
        JsValueRef* arguments,
        unsigned short argCount,
    ...
    static JsValueRef CALLBACK getCountCallback(
        JsValueRef callee,
        bool isConstructor,
        JsValueRef* arguments,
        unsigned short argCount,
    ...
    static JsValueRef CALLBACK getPickupTime(
        JsValueRef callee,
        bool isConstructor,
        JsValueRef* arguments,
        unsigned short argCount,
        ...
        return Chakra::MakeDouble(
            static_cast<double>(
                std::chrono::duration_cast<std::chrono::milliseconds>(
                    item->pickupTime.time_since_epoch()
                )
                    .count()
            )
        );
    ...
    JsValueRef construct(SDK::ItemStack* item, bool del) {
        ...
        if (item->item) {
            Chakra::SetProperty(
                obj,
                L"item",
                this->owner->getClass<JsItem>()->construct(*item->item, false),
                true
            );
        } else {
