  ...
  protected:
    static JsValueRef CALLBACK jsConstructor(
        ...
        if (!Chakra::VerifyParameters(
                {{arguments[1], JsNumber}, {arguments[2], JsNumber}}
            ))
            return JS_INVALID_REFERENCE;

        return thi->construct(
            d2d::Rect(
                (float)Chakra::GetNumber(arguments[1]),
                (float)Chakra::GetNumber(arguments[2]),
                (float)Chakra::GetNumber(arguments[3]),
                (float)Chakra::GetNumber(arguments[4])
            )
        );
    }
    ...
    static JsValueRef CALLBACK toStringCallback(
        ...
        std::string add = std::format(
            "{} (left={:.2}, top={:.2} right={:.2} bottom={:.2})",
            util::WStrToStr(thi->name),
            rect.left,
            rect.top,
            rect.right,
            rect.bottom
  ...
  public:
    ...
    JsValueRef construct(d2d::Rect const& rec) {
        ...
        Chakra::SetPropertyNumber(
            obj,
            L"left",
            static_cast<double>(rec.left),
            true
        );
        Chakra::SetPropertyNumber(
            obj,
            L"top",
            static_cast<double>(rec.top),
            true
        );
        Chakra::SetPropertyNumber(
            obj,
            L"right",
            static_cast<double>(rec.right),
            true
        );
        Chakra::SetPropertyNumber(
            obj,
            L"bottom",
            static_cast<double>(rec.bottom),
            true
        );
        return obj;
    }
