...
JsValueRef JsWorkerThread::loadFile(
    JsValueRef callee,
    bool isConstructor,
    ...
    auto path = util::GetLatitePath() / "Scripts" / thi->owner->relFolderPath
        / Chakra::GetString(arguments[1]);
    std::ifstream ifs;
    ifs.open(path);
    if (ifs.fail()) {
        Chakra::ThrowError(
            L"Could not open file " + path.wstring() + L" for reading"
        );
        return JS_INVALID_REFERENCE;
    }
