...
void JsPlugin::checkTrusted() {
    trusted = false;
    std::string url =
        "https://raw.githubusercontent.com/Imrglop/Latite-Releases/refs/heads/main/script_hashes.txt";
    HttpClient client;
    ...
    if (response.StatusCode() != HttpStatusCode::Ok) {
        Logger::Info(
            "Plugin trust check request failed: {}",
    ...
    std::string stringResponse =
        to_string(response.Content().ReadAsStringAsync().get());
...
bool JsPlugin::load() {
    ...
    if (!fetchPluginData()) {
        Logger::Warn(
            "Could not load plugin metadata from the plugin metadata file."
        );
    }

    if (JS::JsCreateRuntime(JsRuntimeAttributeNone, nullptr, &this->runtime)
        != JsNoError)
        return false;

    this->mainScript = loadAndRunScript(
        std::wstring(MAIN_SCRIPT_NAME.data(), MAIN_SCRIPT_NAME.size())
    );
...
bool JsPlugin::fetchPluginData() {
    ...
    if (name.is_string())
        this->name = util::StrToWStr(name.get<std::string>());
    if (author.is_string())
        this->author = util::StrToWStr(author.get<std::string>());
    if (version.is_string())
        this->version = util::StrToWStr(version.get<std::string>());
    if (desc.is_string())
        this->description = util::StrToWStr(desc.get<std::string>());
...
std::shared_ptr<JsScript> JsPlugin::loadAndRunScript(std::wstring relPath) {
    auto scr =
        std::make_shared<JsScript>(this, this->getPath() / relPath, relPath);
    scr->load();

    JsValueRef global;
    JS::JsGetGlobalObject(&global
    ); // this doesn't add a reference that needs to be freed
    ...
    Chakra::SetProperty(
        ...
        L"exports",
        exportsObj
    ); // So the object.setproperty(exports, esmodule)... works

    Chakra::SetProperty(
        modObj,
        L"exports",
        Chakra::GetProperty(modObj, L"exports")
    ); // This should just reference module.exports, but idk
...
std::shared_ptr<JsScript>
JsPlugin::loadOrFindModule(JsScript* script, std::wstring name) {
...
JsPlugin::getHash(std::filesystem::path const& main) {
    ...
    std::function<void(std::filesystem::path const&)> iterate =
        [&jsFiles, &iterate](std::filesystem::path const& path) -> void {
        for (auto& fil : std::filesystem::directory_iterator(path)) {
            if (fil.is_directory()) {
                iterate(fil);
                continue;
            }
            if (fil.path().string().ends_with(XOR_STRING(".js")
                )) { // I think I should still be fine with unicode filepaths
    ...
    for (auto& fil : jsFiles) {
        ...
        if (!ifs.fail()) {
            ...
            Logger::Warn(
                "[Script] Error opening script file {} to get hash: {}",
                fil.string(),
                errno
            );
        }
