...
void Graphics3DScriptingObject::initialize(
    JsContextRef ctx,
    JsValueRef parentObj
) {
    ...
    *screenContext->shaderColor = {1.f, 1.f, 1.f, 1.f};
    ...
    for (auto& command : commands) {
        ...
        SDK::MeshHelpers::renderMeshImmediately(
            screenContext,
            tess,
            command.renderThrough ? SDK::MaterialPtr::getSelectionBoxMaterial()
...
JsValueRef Graphics3DScriptingObject::drawLineCallback(
    JsValueRef callee,
    bool isConstructor,
    ...
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsObject}, {arguments[2], JsObject}}
        ))
        return JS_INVALID_REFERENCE;
    ...
    obj->currentCommand.vertexBuffer.push_back(
        Vertex {obj->primaryColor.value_or(obj->colors[0]), p1}
    );
    obj->currentCommand.vertexBuffer.push_back(
        Vertex {obj->primaryColor.value_or(obj->colors[1]), p2}
    );

    return Chakra::GetUndefined();
}

JsValueRef Graphics3DScriptingObject::drawTriangleCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    obj->currentCommand.vertexBuffer.push_back(
        Vertex {obj->primaryColor.value_or(obj->colors[2]), p3}
    );

    return Chakra::GetUndefined();
}

JsValueRef Graphics3DScriptingObject::drawQuadCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    obj->currentCommand.vertexBuffer.push_back(
        Vertex {obj->primaryColor.value_or(obj->colors[3]), p4}
    );

    return Chakra::GetUndefined();
}

JsValueRef Graphics3DScriptingObject::finishCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyParameters(
            {{Chakra::TryGet(arguments, argCount, 1), JsBoolean, true}}
        ))
        return JS_INVALID_REFERENCE;
...
JsValueRef Graphics3DScriptingObject::setColorCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef Graphics3DScriptingObject::setColorsCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
