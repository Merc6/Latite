...
JsValueRef D2DScriptingObject::useCallback(
    JsValueRef callee,
    bool isConstructor,
    ...
    if (!Chakra::VerifyArgCount(argCount, 2, true))
...
JsValueRef D2DScriptingObject::fillRectCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    thi->operationsDirty.push_back(
        {thi->matrix, OpFillRect(rect, color, radius)}
    );

    return Chakra::GetUndefined();
}

JsValueRef D2DScriptingObject::drawRectCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 4, true, true))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsObject},
             {arguments[2], JsObject},
    ...
    auto mLock = thi->lock();
    thi->operationsDirty.push_back(
        {thi->matrix, OpDrawRect(rect, color, thickness)}
    );

    return Chakra::GetUndefined();
}

JsValueRef D2DScriptingObject::drawTextCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 5, true))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsObject},
...
JsValueRef D2DScriptingObject::drawTextFullCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 7, true))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsObject},
             {arguments[2], JsString},
...
JsValueRef D2DScriptingObject::drawImageCallback(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 6, true))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsObject},
             {arguments[2], JsObject},
    ...
    auto thi = reinterpret_cast<D2DScriptingObject*>(callbackState);
    if (thi->usingMinecraftRend() && thi->cachedCtx && texture
        && texture->getTexture()) {
        MCDrawUtil dc {thi->cachedCtx, Latite::get().getFont()};
    ...
    thi->operationsDirty.push_back(
        {thi->matrix, OpDrawImage {texture, pos, sx, sy, color}}
    );

    return Chakra::GetUndefined();
}

JsValueRef D2DScriptingObject::getTextSize(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 3, true))
        return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
...
JsValueRef D2DScriptingObject::setClippingRect(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef D2DScriptingObject::restoreClippingRect(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef D2DScriptingObject::drawTriangle(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef D2DScriptingObject::fillTriangle(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef D2DScriptingObject::drawCircle(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef D2DScriptingObject::fillCircle(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef D2DScriptingObject::drawLine(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef D2DScriptingObject::drawItem(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 5))
        return Chakra::GetUndefined();
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsObject},
             {arguments[2], JsObject},
    ...
    if (thi->usingMinecraftRend()) {
        ...
        dc.drawItem(
            item,
            pos,
            static_cast<float>(Chakra::GetNumber(arguments[3])),
...
void D2DScriptingObject::DrawVisitor::operator()(OpDrawImage& op) {
    if (!op.texture->getBitmap())
        return;
    D2DUtil dc;
    dc.ctx->DrawBitmap(
        op.texture->getBitmap(),
        {op.pos.x, op.pos.y, op.pos.x + op.sx, op.pos.y + op.sy},
        op.col.a
    );
}
