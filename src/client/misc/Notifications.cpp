...
Notifications::Notifications() {
    Eventing::get().listen<RenderOverlayEvent>(
        this,
        (EventListenerFunc)&Notifications::onRender,
        10
    );
}
...
void Notifications::onRender(Event& evG) {
    ...
    if (!toasts.empty()) {
        ...
        auto dur = static_cast<float>(
            std::chrono::duration_cast<std::chrono::milliseconds>(
                now - toast.createTime
            )
                .count()
        );
        ...
        float transitionOut =
            static_cast<float>(stay + transition * 2) - transition;
        float transl = 1.f;
        if (transitionIn > 0) {
            // in
        ...
        dc.fillRoundedRectangle(
            rec,
            bgCol.asAlpha(opacity * bgCol.a),
            rec.getHeight() / 3.f
        );
        dc.drawRoundedRectangle(
            rec,
            outlineCol.asAlpha(opacity * outlineCol.a),
            rec.getHeight() / 3.f,
            3,
