...
void PacketHooks::PacketSender_sendToServer(
    SDK::PacketSender* sender,
    SDK::Packet* packet
) {
    ...
    SendToServerHook->oFunc<decltype(&PacketSender_sendToServer)>()(
        sender,
        packet
    );
}

std::shared_ptr<SDK::Packet>
PacketHooks::MinecraftPackets_createPacket(SDK::PacketID packetId) {
    auto genPacket =
        CreatePacketHook->oFunc<decltype(&MinecraftPackets_createPacket)>()(
            packetId
        );
...
void PacketHooks::PacketHandlerDispatcherInstance_handle(
    void* instance,
    ...
    if (Latite::isMainThread()) {
        ...
        if (packetId == SDK::PacketID::CHANGE_DIMENSION) {
            ...
            auto pkt =
                std::static_pointer_cast<SDK::ModalFormRequestPacket>(packet);
            ...
            PluginManager::Event sEv {
                XW("modal-form-request"),
                {formId, formJson},
                false
            };
            ...
            isChat.val =
                (pkt->type == SDK::TextPacketType::CHAT
                 || pkt->type == SDK::TextPacketType::RAW
                 || pkt->type == SDK::TextPacketType::SYSTEM_MESSAGE
                 || pkt->type == SDK::TextPacketType::WHISPER
                 || pkt->type == SDK::TextPacketType::OBJECT_WHISPER
                 || pkt->type == SDK::TextPacketType::ANNOUNCEMENT);

            PluginManager::Event sEv {
                L"receive-chat",
                {typ, val, val2, val3, isChat},
                true
            };
            if (Latite::getPluginManager().dispatchEvent(sEv)) {
                return;
    ...
    hook->oFunc<decltype(&PacketHandlerDispatcherInstance_handle)>()(
        instance,
        networkIdentifier,
        netEventCallback,
        packet
...
PacketHooks::PacketHooks() {
    ...
    for (uint8_t i = 1; i < (uint8_t)SDK::PacketID::COUNT; i++) {
        ...
        if (pkt) {
            ...
            PacketHookArray[i] = addTableSwapHook(
                (uintptr_t)(vft + 1),
                &PacketHandlerDispatcherInstance_handle,
                "Packet Hook"
            );
        }
    }
...
void PacketHooks::initPacketSender(SDK::PacketSender* sender) {
    ...
    SendToServerHook = addTableSwapHook(
        (uintptr_t)(vtable + 2),
