...
#endif
{
    ...
    if (tableSwap) {
        auto res =
            vh::hook(reinterpret_cast<LPVOID*>(target), detour, &this->funcPtr);
        if (res != 0) {
#ifdef LATITE_DEBUG
            Logger::Warn(
                "Creation of hook {} failed with status {}",
                this->funcName,
                vh::status_to_string(res)
            );
#endif
        }
        return;
    ...
    MH_STATUS res =
        MH_CreateHook(reinterpret_cast<LPVOID>(target), detour, &this->funcPtr);
    if (res != MH_OK) {
#ifdef LATITE_DEBUG
        Logger::Warn(
            "Creation of hook {} failed with status {}",
...
std::shared_ptr<Hook>
HookGroup::addHook(uintptr_t ptr, func_ptr_t detour, const char* name) {
#ifdef LATITE_DEBUG
...
std::shared_ptr<Hook> HookGroup::addTableSwapHook(
    uintptr_t ptr,
    func_ptr_t detour,
    const char* name
) {
