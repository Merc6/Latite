...
Screen::Screen() {
	...
	*/
    ...
    Eventing::get()
        .listen<UpdateEvent>(this, (EventListenerFunc)&Screen::onUpdate, 0);
    Eventing::get().listen<RenderOverlayEvent>(
        this,
        (EventListenerFunc)&Screen::onRenderOverlay,
        0,
        true
    );
    Eventing::get()
        .listen<ClickEvent>(this, (EventListenerFunc)&Screen::onClick, 3, true);
}
...
void Screen::onClick(Event& evGeneric) {
    ...
    if (ev.getMouseButton() > 0) {
        if (ev.getMouseButton() < 4) {
            if (isActive()) {
                if (ev.isDown())
                    this->activeMouseButtons[ev.getMouseButton() - 1] =
                        ev.isDown();
...
void Screen::onRenderOverlay(Event& ev) {
    ...
    if (isActive() && this->tooltip.has_value()) {
        ...
        if (now - lastTooltipChange >= 500ms) {
            ...
            d2d::Rect textRect = dc.getTextRect(
                this->tooltip.value(),
