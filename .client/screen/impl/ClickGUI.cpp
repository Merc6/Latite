...
ClickGUI::ClickGUI() {
    ...
    Eventing::get().listen<RenderOverlayEvent>(
        this,
        (EventListenerFunc)&ClickGUI::onRender,
        1,
        true
    );
    Eventing::get().listen<RendererCleanupEvent>(
        this,
        (EventListenerFunc)&ClickGUI::onCleanup,
        1,
        true
    );
    Eventing::get().listen<RendererInitEvent>(
        this,
        (EventListenerFunc)&ClickGUI::onInit,
        1,
        true
    );
    Eventing::get()
        .listen<KeyUpdateEvent>(this, (EventListenerFunc)&ClickGUI::onKey, 1);
    Eventing::get()
        .listen<ClickEvent>(this, (EventListenerFunc)&ClickGUI::onClick, 1);
}
...
void ClickGUI::onRender(Event&) {
    ...
    if (mods.empty() || (Latite::getModuleManager().size() != lastCount)) {
        ...
        Latite::getModuleManager().forEach([&](std::shared_ptr<IModule> mod) {
            if (mod->isVisible()) {
                ModContainer
                    container {mod->getDisplayName(), mod->desc(), L"", mod};
                mods.emplace_back(container);
            }
            return false;
        });
    ...
    if (Latite::get().getMenuBlur())
        dc.drawGaussianBlur(
            Latite::get().getMenuBlur().value() * (isActive() ? 1.f : calcAnim)
        );
    ...
    if (isActive()) {
        ...
        dc.ctx->SetTransform(
            D2D1::Matrix3x2F::Scale(
                {calcAnim, calcAnim},
                D2D1_POINT_2F(rect.center().x, rect.center().y)
            )
        );
    ...
    calcAnim = std::lerp(
        calcAnim,
        isActive() ? 1.f : 0.f,
        Latite::getRenderer().getDeltaTime() * 0.2f
    );
    ...
    shadowEffect->SetValue(
        ...
        D2D1::Vector4F(0.f, 0.f, 0.f, 0.1f)
    );
    auto affineTransformEffect =
        Latite::getRenderer().getAffineTransformEffect();

    D2D1::Matrix3x2F mat =
        D2D1::Matrix3x2F::Translation(10.f * adaptedScale, 5.f * adaptedScale);
    ...
    D2D1_RECT_F logoRect = {
        rect.left + offX,
        rect.top + offY,
        rect.left + offX + imgSize,
        rect.top + offY + imgSize
    };
    ...
    // Latite Logo + text
    {
        ...
        dc.drawText(
            {logoRect.right + 9.f * adaptedScale,
             logoRect.top,
             logoRect.right + 500.f,
    ...
    // X button / other menus
    {
        ...
        RectF xRect = {
            rect.right - xOffs - xWidth,
            rect.top + yOffs,
            rect.right - xOffs,
            rect.top + yOffs + xHeight
        };
        ...
        if (tab == SETTINGS) {
            ...
            RectF backArrowRect =
                {right - xWidth, xRect.top, right, xRect.bottom};
            {
                dc.ctx->DrawBitmap(
                    Latite::getAssets().arrowBackIcon.getBitmap(),
                    backArrowRect
                );
            }
            ...
            RectF hudEditRect;
            {
                ...
                hudEditRect = {
                    hudEditRight - hudEditWidth,
                    xRect.bottom - hudEditHeight,
                    hudEditRight,
                    xRect.bottom
                };

                dc.ctx->DrawBitmap(
                    Latite::getAssets().hudEditIcon.getBitmap(),
                    hudEditRect
                );
            ...
            RectF settingsRect;
            {
                ...
                dc.ctx->DrawBitmap(
                    Latite::getAssets().cogIcon.getBitmap(),
                    settingsRect
                );
            }
        }
    ...
    RectF searchRect {};
    {
        ...
        searchRect = {
            logoRect.left,
            logoRect.bottom + gapY,
            logoRect.left + searchWidth,
            logoRect.bottom + gapY + searchHeight
        };
...

        {
            ...
            D2D1::Matrix3x2F matr = D2D1::Matrix3x2F::Translation(
                5 * adaptedScale,
                5 * adaptedScale
            );
            affineTransformEffect->SetValue(
            ...
            shadowEffect->SetValue(
                D2D1_SHADOW_PROP_COLOR,
                D2D1::Vector4F(0.f, 0.f, 0.f, 0.4f)
            );
            shadowEffect->SetValue(
            ...
            compositeEffect->SetInput(1, shadowBitmap.Get());
            {
                ...
                Vec2 ts = dc.getTextSize(
                    searchTextBox.getText()
                        .substr(0, searchTextBox.getCaretLocation()),
                    Renderer::FontSelection::PrimaryRegular,
                    searchRect.getHeight() / 2.f
                );
                RectF textSearchRect = {
                    searchRect.left + 5.f + searchRect.getHeight(),
                    searchRect.top,
                    searchRect.right - 5.f + searchRect.getHeight(),
                    searchRect.bottom
                };
                d2d::Rect blinkerRect = {
                    textSearchRect.left + ts.x,
                    searchRect.top + 3.f,
                    textSearchRect.left + ts.x + 2.f,
                    searchRect.bottom - 3.f
                };
                ...
                dc.ctx->DrawBitmap(
                    Latite::getAssets().searchIcon.getBitmap(),
                    {searchRect.left + 10.f,
                     searchRect.top + 6.f,
        ...
        if (tab == SETTINGS) {
                ...
                settings.forEach([&](std::shared_ptr<Setting> set) {
                    if (setPos.y <= rect.bottom) {
                        if (set->visible && set->shouldRender(settings)
                            && set->value->index()
                                == (size_t)Setting::Type::
                                    Float /* || set->value->index() == Setting::Type::Int*/) {
                            setPos.y = drawSetting(
                                           set.get(),
                                           &settings,
                                           setPos,
                                           dc,
            ...
            setPos.y += padToSettings;
            {
                ...
                settings.forEach([&](std::shared_ptr<Setting> set) {
                    if (setPos.y <= rect.bottom) {
                        if (set->visible && set->shouldRender(settings)
                            && (set->value->index()
                                    == (size_t)Setting::Type::Key
                                || set->value->index()
                                    == (size_t)Setting::Type::Enum
                                || set->value->index()
                                    == (size_t)Setting::Type::Color
                                || set->value->index()
                                    == (size_t)Setting::Type::Text)) {
                            setPos.y = drawSetting(
                                           set.get(),
                                           &settings,
                                           setPos,
            ...
            // bool settings
            setPos = {
                rect.left + rect.getWidth() * (1.3f / 3.f),
                ...
                settings.forEach([&](std::shared_ptr<Setting> set) {
                    if (setPos.y <= rect.bottom) {
                        if (set->visible && set->shouldRender(settings)
                            && set->value->index()
                                == (size_t)Setting::Type::
                                    Bool /* || set->value->index() == Setting::Type::Enum*/) {
                            setPos.y = drawSetting(
                                           set.get(),
                                           &settings,
                                           setPos,
            ...
            static std::vector<
                std::tuple<std::wstring, ClickGUI::ModTab, d2d::Color, float>>
                modTabs = {
                    {L"All", ALL, searchCol, 0.f},
                    {L"Game", GAME, searchCol, 0.f},
                    {L"HUD", HUD, searchCol, 0.f},
                    {L"Plugins", SCRIPT, searchCol, 0.f}
                };

            float nodeWidth = guiWidth * 0.083f;

            RectF nodeRect = {
                searchRect.right + gaps,
                searchRect.top,
                searchRect.right + gaps + nodeWidth,
                searchRect.bottom
            };
            ...
            for (auto& pair : modTabs) {
                ...
                renderTabRect =
                    renderTabRect.translate(0.f, pressDownTranslate);

                bool contains = shouldSelect(renderTabRect, cursorPos);
                std::get<2>(pair) = util::LerpColorState(
                    std::get<2>(pair),
                    searchCol + 0.2f,
                    searchCol,
                    contains
                ...
                std::get<3>(pair) = std::lerp(
                    std::get<3>(pair),
                    ((contains && mouseButtons[0])
                     || modTab == std::get<1>(pair))
                        ? 1.f
                        : 0.f,
                ...
                dc.drawText(
                    renderTabRect,
                    std::get<0>(pair),
                    {baseColor, baseColor, baseColor, 0.8f},
                    FontSelection::PrimaryRegular,
                    nodeRect.getHeight() / 2.f,
    ...
    if (this->tab == MODULES) {
        ...
        dc.ctx->PushAxisAlignedClip(
            {rect.left, y, rect.right, rect.bottom},
        ...
        for (auto& mod : mods) {
            ...
            if (modTab == GAME && mod.mod->getCategory() == IModule::HUD)
                mod.shouldRender = false; // Game Tab
            if (modTab == HUD && !mod.mod->isHud())
                mod.shouldRender = false; // Hud Tab
            if (modTab == SCRIPT && mod.mod->getCategory() != IModule::SCRIPT)
                mod.shouldRender = false; // Hud Tab
            if (modTab == SCRIPT && mod.isMarketScript)
                mod.shouldRender = true;
            ...
            if (this->searchTextBox.getText().size() > 0) {
                ...
                if (lower.rfind(lowerSearch) != UINTPTR_MAX)
                    should = true;
            }
        ...
        for (auto& mod : mods) {
            ...
            if (mod.modRect.has_value()) {
                if (mod.modRect->bottom < rect.top
                    || mod.modRect->top > rect.bottom) {
...

            {
                ...
                RectF toggleRect = {
                    modRect.right - togglePad - toggleWidth,
                    modRect.top + togglePad,
                    modRect.right - togglePad,
                    modRect.bottom - togglePad
                };
                ...
                if (renderExtended) {
                    // clipping pane
                    {
                        ...
                        RectF descTextRect = {
                            modRect.left + rlBounds,
                            modRect.bottom,
                            toggleRect.left,
                            modRect.bottom + textSizeDesc + descTextPad
                        };
                        descTextRect.bottom = descTextRect.top
                            + dc.getTextSize(
                                    mod.description,
                                    Renderer::FontSelection::PrimaryRegular,
                                    textSizeDesc,
                                    ...
                                    true,
                                    Vec2 {
                                        descTextRect.getWidth(),
                                        descTextRect.getHeight()
                                    }
                            )
                                  .y
                            + descTextPad;
...

                        {
                            ...
                            RectF resetRect = {
                                ...
                                descTextRect.top,
                                toggleRect.right,
                                descTextRect.top + toggleRect.getHeight()
                            };

                            dc.drawRoundedRectangle(
                                resetRect,
                                d2d::Color::RGB(0xFB, 0x36, 0x36),
                                resetRect.getHeight() * (0.223f),
                                0.5f,
                            ...
                            dc.drawText(
                                ...
                                d2d::Color::RGB(0xFB, 0x36, 0x36),
                                Renderer::FontSelection::PrimaryRegular,
                                resetRect.getHeight() * 0.6f,
                                DWRITE_TEXT_ALIGNMENT_CENTER,
                                DWRITE_PARAGRAPH_ALIGNMENT_CENTER
                            );

                            if (resetRect.contains(cursorPos)
                                && justClicked[0]) {
                                mod.mod->settings->forEach(
                                    [&](std::shared_ptr<Setting> set) {
                        ...
                        float settingHeight =
                            rect.getHeight() * setting_height_relative;
                            ...
                            [&](std::shared_ptr<Setting> set) {
                                if (set->visible) {
                                    if (modRectActual.bottom <= rect.bottom) {
                                        if (set->shouldRender(
                                                *mod.mod->settings.get()
                                            )) {
                                            float newY = drawSetting(
                                                set.get(),
                                                mod.mod->settings.get(),
                                                {descTextRect.left,
                                            ...
                                            modRectActual.bottom =
                                                (newY - modRectActual.bottom)
                                                    > 2.f
                                                ? (newY
                                                   + setting_height_relative
                                                       * rect.getHeight()
                                                       * 1.6f)
                                                : modRectActual.bottom;
                                        }
                        ...
                        if (mod.mod->isHud()
                            && static_cast<HUDModule*>(mod.mod.get())
                                   ->isShowPreview()) {
                ...
                if (renderExtended) {
                    modRectActual.bottom =
                        (modRect.bottom
                         + (modRectActual.getHeight() - modRect.getHeight())
                             * (1.f - mod.lerpArrowRot));
                ...
                dc.drawRoundedRectangle(
                    modRectActual,
                    d2d::Color(Latite::get().getAccentColor().getMainColor())
                        .asAlpha(1.f * mod.lerpToggle),
                    .22f * modHeight,
                    1.f,
                    DrawUtil::OutlinePosition::Inside
                );
                ...
                if (mod.mod) {
                    if (mod.mod->shouldHoldToToggle()) {
                        d2d::Color color =
                            d2d::Color::RGB(0xD9, 0xD9, 0xD9, 30);

                        dc.fillRoundedRectangle(
                            toggleRect,
                            color,
                            toggleRect.getHeight() / 4.f
                        );
                        dc.drawText(
                            toggleRect,
                            util::StrToWStr(
                                util::KeyToString(mod.mod->getKeybind())
                            ),
                            {1.f, 1.f, 1.f, 1.f},
                            Renderer::FontSelection::PrimaryRegular,
                        ...
                        if (this->shouldSelect(toggleRect, cursorPos)) {
                            setTooltip(
                                L"Enable this module using the keybind."
                            );
                        }
                        ...
                        bool selecToggle;
                        if (selecToggle =
                                this->shouldSelect(toggleRect, cursorPos)) {
                            if (justClicked[0]) {
                        ...
                        mod.toggleColorOn = util::LerpColorState(
                            mod.toggleColorOn,
                            d2d::Color(
                                Latite::get().getAccentColor().getMainColor()
                            ) + 0.2f,
                            d2d::Color(
                                Latite::get().getAccentColor().getMainColor()
                            ),
                        ...
                        mod.toggleColorOff = util::LerpColorState(
                            mod.toggleColorOff,
                            offCol + 0.2f,
                            offCol,
                            selecToggle
                        ...
                        dc.fillRoundedRectangle(
                            toggleRect,
                            mod.mod->isEnabled() ? mod.toggleColorOn
                                                 : mod.toggleColorOff,
                            toggleRect.getHeight() / 2.f
                        );
                        Vec2 center {
                            toggleRect.left + circleOffs,
                            toggleRect.centerY()
                        };
                        Vec2 center2 = center;
                        ...
                        mod.lerpToggle = std::lerp(
                            mod.lerpToggle,
                            mod.mod->isEnabled() ? 1.f : 0.f,
                        ...
                        dc.ctx->FillEllipse(
                            D2D1::Ellipse({center.x, center.y}, radius, radius),
                            dc.brush
                        );
                    }
                }
                ...
                RectF arrowRc = {
                    modRect.left + (modRect.getHeight() * 0.4f),
                ...
                // arrow
                if (mod.mod) {
                    if (this->shouldSelect(modRect, cursorPos)
                        && !shouldSelect(toggleRect, cursorPos)) {
                        if (justClicked[0]) {
                    ...
                    dc.ctx->SetTransform(
                        D2D1::Matrix3x2F::Rotation(
                            (1.f - mod.lerpArrowRot) * 180.f,
                            {arrowRc.centerX(), arrowRc.centerY()}
                        )
                        * oMatr
                    );
                    mod.lerpArrowRot = std::lerp(
                        mod.lerpArrowRot,
                        toLerp,
                        Latite::getRenderer().getDeltaTime() * 0.3f
                    );
                    // icon
                    dc.ctx->DrawBitmap(
                        Latite::getAssets().arrowIcon.getBitmap(),
                        arrowRc.get()
                    );
                    dc.ctx->SetTransform(oMatr);
                } else if (mod.isMarketScript) {
                    dc.ctx->DrawBitmap(
                        Latite::getAssets().document.getBitmap(),
                        arrowRc.get()
                    );
            ...
            mod.lerpHover = std::lerp(
                mod.lerpHover,
                shouldSelect(modRectActual, cursorPos) ? 1.f : 0.f,
                Latite::getRenderer().getDeltaTime() / 5.f
            );
            ...
            float scrollYNew =
                std::max(
                    0.f,
                    (modRectActual.bottom + padFromSearchBar) - rect.bottom
                )
                + lerpScroll;
            if (scrollYNew > scrollMax)
                scrollMax = scrollYNew;
            if (i >= (numMods - 1)) {
                i = 0;
                row++;
                column = 0;
    ...
    if (colorPicker.setting) {
        ...
        if (colorPicker.queueClose) {
            ...
            auto d2dCol = d2d::Color(util::HSVToColor(colorPicker.pickerColor))
                              .asAlpha(colorPicker.opacityMod);
            *colorPicker
                 .selectedColor = {d2dCol.r, d2dCol.g, d2dCol.b, d2dCol.a};
...
void ClickGUI::onClick(Event& evGeneric) {
    ...
    if (ev.getMouseButton() == 4) {
        ...
        this->scroll = std::max(
            std::min(
                scroll - static_cast<float>(ev.getWheelDelta()) / 3.f,
                scrollMax
            ),
            0.f
        );
...
void drawAlphaBar(D2DUtil& dc, d2d::Rect rect, float nodeSize, int rows) {
    ...
    for (int i = 0; i < rows; i++) {
        if (i % 2 == 0) {
            for (float beginX = rect.left; beginX < rect.right;
                 beginX += bs * 2.f) {
            ...
            for (float beginX = rect.left + bs; beginX < rect.right;
                 beginX += bs * 2.f) {
                float endX = std::min(rect.right, beginX + bs);
                dc.fillRectangle(
                    {beginX, beginY, endX, endY},
...
float ClickGUI::drawSetting(
    ...
    switch ((Setting::Type)((*set->value).index())) {
        case Setting::Type::Text: {
            ...
            textVal.str = std::move(tb->getText()
            ); // I think im supposed to std::move this?
            dc.drawText(
                rightRc,
                set->getDisplayName(),
                {1.f, 1.f, 1.f, 1.f},
                Renderer::FontSelection::PrimarySemilight,
                textSize,
                DWRITE_TEXT_ALIGNMENT_LEADING,
        ...
        case Setting::Type::Bool: {
            RectF checkboxRect =
                {pos.x, pos.y, pos.x + checkboxSize, pos.y + checkboxSize};
            bool contains = bypassClickThrough
                ? checkboxRect.contains(cursorPos)
                : this->shouldSelect(checkboxRect, cursorPos);
            ...
            auto lerpedColor = util::LerpColorState(
                set->rendererInfo.col,
                colOff + 0.1f,
                colOff,
                contains
            );
            ...
            if (std::get<BoolValue>(*set->value)) {
                ...
                RectF markRect = {
                    checkboxRect.left + checkWidth / 4.f,
                    checkboxRect.top + checkHeight / 2.f,
                    checkboxRect.right - checkWidth / 4.f,
                    checkboxRect.bottom - checkHeight / 2.f
                };

                dc.ctx->DrawBitmap(
                    Latite::getAssets().checkmarkIcon.getBitmap(),
                    markRect
                );
            }
            ...
            auto desc = set->desc();
            if (!desc.empty())
                if (contains || shouldSelect(textRect, cursorPos))
                    setTooltip(desc);
        ...
        case Setting::Type::Key: {
            ...
            std::wstring text = util::StrToWStr(
                util::KeyToString(std::get<KeyValue>(*set->value))
            );
            ...
            auto ts = dc.getTextSize(
                          text,
                          FontSelection::PrimaryRegular,
                          textSize * 0.9f
                      )
                + Vec2(8.f, 0.f);
            if (ts.x > keyRect.getWidth())
                keyRect.right = keyRect.left + (ts.x);
            ...
            if (!set->desc().empty())
                if (shouldSelect(textRect, cursorPos))
                    setTooltip(set->desc());
        ...
        case Setting::Type::Enum: {
            ...
            auto ts = dc.getTextSize(
                ...
                + Vec2(8.f, 0.f);
            if (ts.x > enumRect.getWidth())
                enumRect.right = enumRect.left + (ts.x);
        ...
        case Setting::Type::Color: {
            ...
            d2d::Color col = {
                colVal.getMainColor().r,
                colVal.getMainColor().g,
                colVal.getMainColor().b,
                colVal.getMainColor().a
            };

            const D2D1_GRADIENT_STOP stops[] =
                {0.f, col.asAlpha(1.f).get(), 1.f, col.get()};

            dc.ctx->CreateGradientStopCollection(
                stops,
                _countof(stops),
                gradientStopCollection.GetAddressOf()
            );
            dc.ctx->CreateLinearGradientBrush(
                prop,
            ...
            if (shouldSelect(colRect, cursorPos)) {
                if (justClicked[0]) {
                    ...
                    std::get<BoolValue>(colorPicker.rgbSelector) =
                        std::get<ColorValue>(*set->value).isRGB;
                    colorPicker.dragging = false;
                    cPickerRect =
                        {colRect.left, colRect.bottom + 20.f, 0.f, 0.f};
                    auto& colVal = std::get<ColorValue>(*set->value);
                    colorPicker.selectedColor = &colVal.color1;
                    auto sCol = *colorPicker.selectedColor;
                    colorPicker.pickerColor =
                        util::ColorToHSV({sCol.r, sCol.g, sCol.b, sCol.a});
        ...
        case Setting::Type::Float: {
            ...
            RectF textRect =
                {pos.x, pos.y, pos.x + textWidth, pos.y + sliderHeight};
            RectF rtTextRect =
                textRect.translate(0.f, -(textRect.getHeight() / 2.f));
            ...
            RectF sliderRect = {
                textRect.right,
                sliderTop,
                textRect.left + size - (0.1947f * size),
                sliderTop + sliderHeight
            };

            float innerPad = 0.2f * sliderRect.getHeight();
            RectF innerSliderRect = {
                sliderRect.left + innerPad,
                sliderRect.top + innerPad,
                sliderRect.right - innerPad,
                sliderTop + (rect.getHeight() * 0.017730f) - innerPad
            };
            ...
            RectF rtRect =
                rightRect.translate(0.f, -(sliderRect.getHeight() / 2.f));
            dc.drawText(
                rtRect,
                valuew.str(),
                d2d::Color(1.f, 1.f, 1.f, 1.f),
            ...
            if (!set->desc().empty()
                && (shouldSelect(textRect, cursorPos)
                    || shouldSelect(sliderRect, cursorPos))) {
                setTooltip(set->desc());
            }
            ...
            if (!this->activeSetting) {
                ...
                if (activeSetting == set) {
                    ...
                    float newVal =
                        percent * (std::get<FloatValue>(set->max) - min);
                    newVal += min;
            ...
            dc.fillRoundedRectangle(
                sliderRect,
                d2d::Color::RGB(0x8D, 0x8D, 0x8D).asAlpha(0.11f),
                sliderRect.getHeight() / 2.f
            );
            dc.fillRoundedRectangle(
                innerSliderRect,
                d2d::Color(Latite::get().getAccentColor().getMainColor()),
                innerSliderRect.getHeight() / 2.f
            );

            dc.brush->SetColor(d2d::Color(0xB9, 0xB9, 0xB9).get());
            dc.ctx->FillEllipse(
                D2D1::Ellipse(
                    {innerSliderRect.right, sliderRect.centerY()},
                    sliderRect.getHeight() * 0.6f,
                    sliderRect.getHeight() * 0.6f
                ),
                dc.brush
            );
            return rtTextRect.top
                + dc.getTextSize(
                        namew.str(),
                        Renderer::FontSelection::PrimarySemilight,
                        textSz,
                        false,
                        true,
                        Vec2 {rtTextRect.getWidth(), rtTextRect.getHeight()}
                )
                      .y;
        } break;
        default:
...
void ClickGUI::drawColorPicker() {
    ...
    RectF titleRect = {
        cPickerRect.left + remPad,
        cPickerRect.top + remPad,
        cPickerRect.right - remPad,
...

    {
        dc.drawText(
            ...
            L"Color Picker",
            {1.f, 1.f, 1.f, 1.f},
            Renderer::FontSelection::PrimaryLight,
            textSize,
            DWRITE_TEXT_ALIGNMENT_LEADING,
            DWRITE_PARAGRAPH_ALIGNMENT_CENTER
        );
    ...
    RectF boxRect =
        {titleRect.left, boxTop, titleRect.right, boxTop + boxWidth};
    ...
    d2d::Color baseCol =
        util::HSVToColor({colorPicker.pickerColor.h, 1.f, 1.f});
    ...
    // main brush
    {
        ...
        const D2D1_GRADIENT_STOP stops[] =
            {0.f, {1.f, 1.f, 1.f, 1.f}, 1.f, baseCol.get()};
    ...
    // Value brush
    {
        ...
        const D2D1_GRADIENT_STOP stops[] =
            {0.f, {0.f, 0.f, 0.f, 1.f}, 1.f, {0.f, 0.f, 0.f, 0.f}};
    ...
    dc.drawRectangle(
        boxRect,
        d2d::Color::RGB(0x50, 0x50, 0x50).asAlpha(0.28f),
        2.f
    );
    ...
    // Hue brush
    {
        ...
        const D2D1_GRADIENT_STOP stops[] = {
            0.f,
            d2d::Color(util::HSVToColor({0.f, 1.f, 1.f})).get(),
            1.f / 7.f,
            d2d::Color(util::HSVToColor({(1.f / 7.f) * 360.f, 1.f, 1.f})).get(),
            2.f / 7.f,
            d2d::Color(util::HSVToColor({(2.f / 7.f) * 360.f, 1.f, 1.f})).get(),
            3.f / 7.f,
            d2d::Color(util::HSVToColor({(3.f / 7.f) * 360.f, 1.f, 1.f})).get(),
            4.f / 7.f,
            d2d::Color(util::HSVToColor({(4.f / 7.f) * 360.f, 1.f, 1.f})).get(),
            5.f / 7.f,
            d2d::Color(util::HSVToColor({(5.f / 7.f) * 360.f, 1.f, 1.f})).get(),
            6.f / 7.f,
            d2d::Color(util::HSVToColor({(6.f / 7.f) * 360.f, 1.f, 1.f})).get(),
            1.f,
            d2d::Color(util::HSVToColor({0.f, 1.f, 1.f})).get(),
        };
    ...
    dc.drawRoundedRectangle(
        hueBar,
        d2d::Color::RGB(0x50, 0x50, 0x50).asAlpha(0.28f),
        hueBar.getHeight() / 2.f,
        hueBar.getHeight() / 4.f,
        DrawUtil::OutlinePosition::Outside
    );

    RectF alphaBar = {
        hueBar.left,
        hueBar.bottom + padToHueBar,
        hueBar.right,
    ...
    dc.drawRoundedRectangle(
        alphaBar,
        d2d::Color::RGB(0x37, 0x37, 0x37).asAlpha(0.88f),
        alphaBar.getHeight() / 2.f,
        alphaBar.getHeight() / 3.f,
        DrawUtil::OutlinePosition::Outside
    );

    // color hex edits/displays

    std::array<std::optional<StoredColor>, 3> cols =
        {colVal.getMainColor(), std::nullopt, std::nullopt};
    ...
    for (size_t i = 0; i < cols.size(); ++i) {
        ...
        if (c.has_value()) {
            ...
            float pad = (alphaBar.getWidth() - colorModeWidth - hexBoxWidth
                         - colorDisplayWidth)
                / 3.f;
            ...
            RectF colorModeRect = {
                totalDisplayRect.left,
                totalDisplayRect.top,
                totalDisplayRect.left + colorModeWidth,
                totalDisplayRect.bottom
            };
            RectF hexBox = {
                colorModeRect.right + pad,
                totalDisplayRect.top,
                colorModeRect.right + pad + hexBoxWidth,
                totalDisplayRect.bottom
            };
            RectF colorDisplayRect = {
                totalDisplayRect.right - pad - colorDisplayWidth,
                totalDisplayRect.top,
                totalDisplayRect.right - pad,
                totalDisplayRect.bottom
            };

            if (pickerTextBoxes.size() <= i) {
                pickerTextBoxes.insert(
                    pickerTextBoxes.begin() + i,
                    ui::TextBox(hexBox, 7)
                );
            ...
            dc.fillRoundedRectangle(
                colorDisplayRect,
                col.asAlpha(colorPicker.opacityMod),
                round
            );

            std::wstring alphaTxt =
                util::StrToWStr(std::format("{:.2f}", colorPicker.opacityMod));

            dc.drawText(
                colorDisplayRect,
                alphaTxt,
                (colorPicker.opacityMod < 0.5f
                 || colorPicker.pickerColor.v < 0.5f)
                    ? D2D1::ColorF::White
                    : D2D1::ColorF::Black,
                Renderer::FontSelection::PrimaryRegular,
                colorDisplayRect.getHeight() * 0.5f,
                DWRITE_TEXT_ALIGNMENT_CENTER,
                DWRITE_PARAGRAPH_ALIGNMENT_CENTER
            );
            tb.setRect(hexBox);
            ...
            drawSetting(
                &colorPicker.rgbSetting,
                nullptr,
                {alphaBar.left, alphaBar.bottom + hexBox.getHeight() * 1.5f},
                dc,
                150.f,
                0.21f,
                true
    ...
    // sv
    if (colorPicker.isEditingSV
        || (justClicked[0] && boxRect.contains(cursorPos))) {
        colorPicker.svModX = std::max(
            std::min(cursorPos.x - boxRect.left, boxRect.getWidth())
                / boxRect.getWidth(),
            0.f
        );
        colorPicker.svModY = std::max(
            std::min(cursorPos.y - boxRect.top, boxRect.getHeight())
                / boxRect.getHeight(),
            0.f
        );
    ...
    // hue
    if (colorPicker.isEditingHue
        || (justClicked[0] && hueBar.contains(cursorPos))) {
        colorPicker.hueMod = std::max(
            std::min(cursorPos.x - hueBar.left, hueBar.getWidth())
                / hueBar.getWidth(),
            0.f
        );
    ...
    // alpha
    if (colorPicker.isEditingOpacity
        || (justClicked[0] && alphaBar.contains(cursorPos))) {
        colorPicker.opacityMod = std::max(
            std::min(cursorPos.x - alphaBar.left, alphaBar.getWidth())
                / alphaBar.getWidth(),
            0.f
        );
    ...
    // SV
    {
        auto ellipse = D2D1::Ellipse(
            {boxRect.left + (hueBar.getWidth() * colorPicker.svModX),
    ...
    {
        auto ellipse = D2D1::Ellipse(
            {hueBar.left + (hueBar.getWidth() * colorPicker.hueMod),
             hueBar.centerY()},
            ellipseRadius,
            ellipseRadius
        );
    ...
    {
        auto ellipse = D2D1::Ellipse(
            {alphaBar.left + (alphaBar.getWidth() * colorPicker.opacityMod),
             alphaBar.centerY()},
            ellipseRadius,
            ellipseRadius
        );
    ...
    dc.fillRoundedRectangle(
        cPickerRect,
        d2d::Color::RGB(0x7, 0x7, 0x7).asAlpha(0.8f),
        19.f * adaptedScale
    );
    dc.drawRoundedRectangle(
        cPickerRect,
        d2d::Color::RGB(0, 0, 0).asAlpha(0.28f),
        19.f * adaptedScale,
        4.f * adaptedScale,
        DrawUtil::OutlinePosition::Outside
    );
    ...
    RectF xRect = {
        cPickerRect.right - xWidth * 2.f,
        cPickerRect.top + xWidth,
        cPickerRect.right - xWidth,
    ...
    RectF pickerTopBar =
        {cPickerRect.left, cPickerRect.top, cPickerRect.right, boxRect.top};

    if (!colorPicker.dragging && justClicked[0]
        && pickerTopBar.contains(cursorPos)) {
