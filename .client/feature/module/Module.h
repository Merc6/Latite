...
class Module: public IModule {
  public:
    ...
    explicit Module(
        ...
        IModule(name, description, displayName, category, visible) {
        this->hud = hud;
...

        {
            auto set = std::make_shared<Setting>(
                "enabled",
                LocalizeString::get("client.module.props.enabled.name"),
        ...
        if (keybind != 1) {
            auto set = std::make_shared<Setting>(
                "key",
                LocalizeString::get("client.module.props.key.name"),
    ...
    void
    listen(Listener listener, bool callWhenInactive = false, int priority = 0) {
        Eventing::get().listen<Event>(
            this,
            (EventListenerFunc)listener,
            priority,
            callWhenInactive
        );
    }
    ...
    [[nodiscard]]
    virtual bool forceMinecraftRenderer() {
        return false;
    };

    virtual void loadConfig(SettingGroup& resolvedGroup) override;
    std::shared_ptr<Setting> addSetting(
    ...
    std::shared_ptr<Setting> addEnumSetting(
        std::string const& internalName,
        std::wstring const& displayName,
        std::wstring const& desc,
        EnumData& dat,
    ...
    std::shared_ptr<Setting> addSliderSetting(
        std::string const& internalName,
        std::wstring const& displayName,
        std::wstring const& desc,
        ValueType& val,
