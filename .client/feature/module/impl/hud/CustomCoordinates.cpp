...
CustomCoordinates::CustomCoordinates() :
    ...
    addSetting(
        "showDimension",
        LocalizeString::get(
            "client.textmodule.customCoordinates.showDimension.name"
        ),
        LocalizeString::get(
            "client.textmodule.customCoordinates.showDimension.desc"
        ),
        this->showDimension
    );
}
...
std::wstringstream CustomCoordinates::text(bool isDefault, bool inEditor) {
    ...
    int playerPosY = lroundf(
        localPlayer->getPos().y - 1.62f
    ...
    std::wstring dimensionName =
        util::StrToWStr(localPlayer->dimension->dimensionName);
    ...
    if (std::get<BoolValue>(showDimension)) {
        if (dimensionName == L"Overworld")
            dimensionName = LocalizeString::get(
                "client.textmodule.customCoordinates.dimensionDisplay.overworld.name"
            );
        else if (dimensionName == L"Nether")
            dimensionName = LocalizeString::get(
                "client.textmodule.customCoordinates.dimensionDisplay.nether.name"
            );
        else if (dimensionName == L"TheEnd")
            dimensionName = LocalizeString::get(
                "client.textmodule.customCoordinates.dimensionDisplay.theEnd.name"
            );
        ...
        moduleText = util::FormatWString(
            ...
            {std::to_wstring(playerPosX),
             ...
             LocalizeString::get(
                 "client.textmodule.customCoordinates.dimension.name"
             ),
             dimensionName}
        );
    }
