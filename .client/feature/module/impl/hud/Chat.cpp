...
Chat::Chat() :
    HUDModule(
        "Chat",
        L"Custom Chat",
        L"A custom chat, replacing the vanilla chat.",
        HUD
    ) {
    ...
    addSetting(
        "antiSpam",
        L"Anti Spam",
        L"Prevent spam messages in chat.",
        antiSpam
    );
    addSliderSetting(
        "animSpeed",
        L"Animation Time",
        L"The speed of the animation",
        animationSpeed,
        FloatValue(0.f),
        FloatValue(5.f),
        FloatValue(0.1f)
    ...
    addSliderSetting(
        "messageDuration",
        L"Message Duration",
        L"The duration of the message, excluding the animation",
        messageDuration,
        FloatValue(1.f),
        FloatValue(15.f),
        FloatValue(0.5f)
    ...
    addEnumSetting(
        "anchor",
        L"Anchor",
        L"How the chat window will be anchored",
        anchorData
    );
...
void Chat::render(DrawUtil& ctx, bool isDefault, bool inEditor) {
    ...
    auto messageDuration = std::chrono::milliseconds(
        static_cast<int64_t>(
            std::get<FloatValue>(this->messageDuration).value * 1000
        )
    );
    auto messageAnimDuration = std::chrono::milliseconds(
        static_cast<int64_t>(
            std::get<FloatValue>(this->animationSpeed).value * 1000
        )
    );
    ...
    for (size_t i = 0;
         i < std::min(messages.size(), static_cast<size_t>(maxMessages));
         i++) {
        auto& msg = messages[i];

        auto textWrap =
            [](DrawUtil& util, std::wstring txt, float tSize, float width
            ) -> std::wstring {
            std::wstring ret = txt;
        ...
        auto sineCurve = [](float x) { return sin(x * 0.5f * pi_f); };

        if (tm - msg.timeCreated > messageDuration) {
            msg.animation = sineCurve(
                std::min(
                    1.f,
                    (messageAnimDuration.count()
        ...
        } else if (tm - msg.timeCreated < messageAnimDuration
                   && msg.duplicate < 2) {
            msg.animation = sineCurve(
        ...
        std::wstring text =
            textWrap(ctx, util::StrToWStr(content), textSize, windowWidth);
        auto tSize = ctx.getTextSize(
            text,
            Renderer::FontSelection::PrimaryRegular,
            textSize
        );
        ...
        tRect = {
            (1.f - msg.animation) * -windowWidth,
            y - tSize.y,
            windowWidth * (msg.animation),
            y
        };
        ...
        ctx.drawText(
            tRect,
            text,
            fgCol.asAlpha(msg.animation * fgCol.a),
            Renderer::FontSelection::PrimaryRegular,
            textSize,
