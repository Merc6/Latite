...
void ui::TextBox::onChar(wchar_t character) {
    if (character == '\b' && text.size() > 0) {
        ...
        text.erase(
            text.begin() + std::min(static_cast<int>(text.size()), place)
        );
...
void ui::TextBox::setText(std::wstring const& str) {
    text = str;
    if (place > str.size())
        place = static_cast<int>(str.size());
}
...
void ui::TextBox::render(
    ...
    if (isSelected() && shouldBlink())
        dc.fillRectangle(blinkerRect, textColor);
}
