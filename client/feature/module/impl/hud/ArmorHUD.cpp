...
ArmorHUD::ArmorHUD() :
    HUDModule(
        "ArmorHUD",
        LocalizeString::get("client.hudmodule.armorHud.name"),
        LocalizeString::get("client.hudmodule.armorHud.desc"),
        HUD
    ) {
    mode.addEntry(
        EnumEntry {
            mode_vetical,
            LocalizeString::get("client.hudmodule.armorHud.modeVertical.name")
        }
    );
    mode.addEntry(
        EnumEntry {
            mode_horizontal,
            LocalizeString::get("client.hudmodule.armorHud.modeHorizontal.name")
        }
    );
    addEnumSetting(
        "mode",
        LocalizeString::get("client.hudmodule.armorHud.mode.name"),
        LocalizeString::get("client.hudmodule.armorHud.mode.desc"),
        mode
    );
    addSetting(
        "durability",
        LocalizeString::get("client.hudmodule.armorHud.durability.name"),
        LocalizeString::get("client.hudmodule.armorHud.durability.desc"),
        showDamage
    );
    addSetting(
        "percent",
        LocalizeString::get("client.hudmodule.armorHud.percent.name"),
        LocalizeString::get("client.hudmodule.armorHud.percent.desc"),
        this->percentage,
        "durability"_istrue
    );
    ...
    addSetting(
        "textColor",
        LocalizeString::get("client.hudmodule.armorHud.textColor.name"),
        LocalizeString::get("client.hudmodule.armorHud.textColor.desc"),
        txtColor
    );
}
...
void ArmorHUD::render(DrawUtil& ctxGeneric, bool isDefault, bool inEditor) {
    ...
    if (auto sl =
            plr->supplies->inventory->getItem(plr->supplies->selectedSlot)) {
        items[0] = sl;
    }
    ...
    bool isLeft = this->getRect().centerX()
        > (SDK::ClientInstance::get()->getGuiData()->screenSize.x / 2.f);
            ...
            if (item && item->item) {
                ...
                if (std::get<BoolValue>(showDamage) && mdmg > 0) {
                    ...
                    if (std::get<BoolValue>(percentage))
                        txt = std::to_wstring(
                                  static_cast<int>(
                                      static_cast<float>(dmg)
                                      / static_cast<float>(mdmg) * 100.f
                                  )
                              )
                            + L'%';

                    Vec2 textSize = dc.getTextSize(
                        txt,
    ...
    if (mode.getSelectedKey() == mode_vetical) {
            ...
            if (item && item->item) {
                ...
                if (std::get<BoolValue>(showDamage) && mdmg > 0) {
                    ...
                    if (!isLeft) {
                        ...
                        extra = 6.f;
                        dc.drawText(
                            rc.translate(-rc.getWidth() - extra, 0.f),
                            txt,
                            std::get<ColorValue>(txtColor).getMainColor(),
                            Renderer::FontSelection::PrimaryRegular,
                            textS,
        ...
        rect.bottom =
            rect.top + (itemSize * std::max(1.f, static_cast<float>(numItems)));
    } else {
        ...
        rect.right = rect.left
            + (itemSize * std::max(1.f, static_cast<float>(numItems)));
