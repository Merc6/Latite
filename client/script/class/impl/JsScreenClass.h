...
class JsScreenClass: public JsWrapperClass<JsScreen> {
  protected:
    ...
    static JsValueRef CALLBACK toStringCallback(
        ...
        std::string add =
            std::format("{} ({})", util::WStrToStr(thi->name), scn->getName());
        return Chakra::MakeString(L"[object " + util::StrToWStr(add) + L"]");
    }

    static JsValueRef CALLBACK isActiveCallback(
        JsValueRef callee,
        bool isConstructor,
        JsValueRef* arguments,
        unsigned short argCount,
        ...
        return Get(arguments[0])->isActive() ? Chakra::GetTrue()
                                             : Chakra::GetFalse();
    }

    static JsValueRef CALLBACK getKeyCallback(
        JsValueRef callee,
        bool isConstructor,
        JsValueRef* arguments,
        unsigned short argCount,
    ...
    static JsValueRef CALLBACK setKeyCallback(
        JsValueRef callee,
        bool isConstructor,
        JsValueRef* arguments,
        unsigned short argCount,
    ...
    static JsValueRef CALLBACK isHoverCallback(
        JsValueRef callee,
        bool isConstructor,
        JsValueRef* arguments,
        unsigned short argCount,
        ...
        if (!Chakra::VerifyParameters(
                {{arguments[1], JsObject},
                 {Chakra::TryGet(arguments, argCount, 2), JsObject}}
            ))
            return JS_INVALID_REFERENCE;
        ...
        return Get(arguments[0])->shouldSelect(rect, point)
            ? Chakra::GetTrue()
            : Chakra::GetFalse();
    }
