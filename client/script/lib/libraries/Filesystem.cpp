...
void throwFsError() {
    ...
    Chakra::ThrowError(
        L"Filesystem error code " + std::to_wstring(errno) + L": " + wstr
    );
}
} // namespace

JsValueRef Filesystem::write(
    JsValueRef callee,
    bool isConstructor,
    ...
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString},
             {arguments[2], JsTypedArray},
             {arguments[3], JsFunction}}
        ))
        return undef;

    auto thi = reinterpret_cast<Filesystem*>(callbackState);
    auto op = std::make_shared<FSAsyncOperation>(
        arguments[3],
        [](JsScript::AsyncOperation* op_) {
...
JsValueRef Filesystem::writeSync(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 3))
        return undef;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString}, {arguments[2], JsTypedArray}}
        ))
        return undef;
    ...
    std::wofstream ofs;
    ofs.open(
        thi->getPath(Chakra::GetString(arguments[1])),
...
JsValueRef Filesystem::readSync(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef Filesystem::existsSync(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    return fs::exists(thi->getPath(Chakra::GetString(arguments[1])))
        ? Chakra::GetTrue()
        : Chakra::GetFalse();
}

JsValueRef Filesystem::createDirectorySync(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!fs::create_directory(
        ...
        if (errCode.value() != ERROR_ALREADY_EXISTS && errCode.value() != 0) {
            Chakra::ThrowError(
                L"Filesystem error: " + util::StrToWStr(errCode.message())
            );
        }
        return ret;
    }
...
JsValueRef Filesystem::appendSync(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef Filesystem::deleteFile(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef Filesystem::readdirSync(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
...
JsValueRef Filesystem::moveSync(
    JsValueRef callee,
    bool isConstructor,
    JsValueRef* arguments,
    unsigned short argCount,
    ...
    if (!Chakra::VerifyArgCount(argCount, 3))
        return ret;
    if (!Chakra::VerifyParameters(
            {{arguments[1], JsString}, {arguments[1], JsString}}
        ))
        return ret;
    std::wifstream ifs;
    std::wstringstream wss;
