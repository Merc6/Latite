...
struct proc_module {
    ...
    [[nodiscard]]
    memory_span get_memory_range() const;
    [[nodiscard]]
    std::optional<memory_span> get_section_range(std::string_view name) const;
};

[[nodiscard]]
proc_module get_main_proc_module();
[[nodiscard]]
std::optional<proc_module> get_proc_module(const std::string& name);
